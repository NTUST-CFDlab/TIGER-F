...
subroutine gridder_sin5() ! Double uniform & sine grid spacing method
use variables
implicit none
integer :: ncount
real*8 :: nratio
...
integer, parameter :: nzLrg1 = INT( (nz-nzMid-nzSml)/(lz-lzMid)*lzLrg1 )  !1 
...
!----------------Unequal grid intervals----------------!
    ...
    do j=1,ny+1
        ...
        elseif( j <= nyLrg1+1 ) then                                                                                 !------------------------Sin dyLrg1-----------------------!
            ...
            nratio = (j-1.d0)/nyLrg1*1.d0 
            Y(j) = (lyLrg1-dyMid1*nyLrg1)*(nratio+tune/thetaa*SIN(nratio*thetaa)) + dyMid1*(j-1.d0)

        elseif( j > nyLrg1+1 .AND. j <= nyLrg1+nyMid1+1 ) then                                                       !------------------------Uniform dyMid1-------------------!
        ...
        elseif( j > nyLrg1+nyMid1+nySml+1 .AND. j <= nyLrg1+nyMid1+nySml+nyMid2+1 ) then                             !------------------------Uniform dyMid2-------------------!

            Y(j) = Y(j-1) + dyMid2

        elseif( j > nyLrg1+nyMid1+nySml+nyMid2+1 .AND. j <= ny+1 ) then                                              !------------------------Sin dyLrg2-----------------------!

            nratio = (j-nyLrg1-nyMid-nySml-1.d0)/nyLrg2*1.d0 
			Y(j) = lyLrg1+lyMid + (lyLrg2-dyMid2*nyLrg2)*(nratio-tune/thetaa*SIN(nratio*thetaa)) + &
			dyMid2*(j-nyLrg1-nyMid1-nyMid2-nySml-1.d0)
        endif
    ...
    do k=1,nz+1
        ...
        elseif( k > nzLrg1+nzSml+1 .AND. k <= nzLrg1+nzSml+nzMid2+1 ) then                                          !------------------------Uniform dzMid2-------------------!
            Z(k) = Z(k-1) + dzMid2

        elseif( k > nzLrg1+nzSml+nzMid2+1  .AND. k <= nz+1 ) then                                                   !------------------------Sin dzLrg2-----------------------!

            nratio = (k-nzLrg1-nzSml-nzMid2-1.d0)/nzLrg2*1.d0 
			Z(k) =  lzLrg1+lzSml+lzMid2 + (lzLrg2-dzMid2*nzLrg2)*(nratio-tune/thetaa*SIN(nratio*thetaa)) + &
			dzMid2*(k-nzLrg1-nzSml-nzMid2-1.d0)
