...
implicit none
   ...
   real*8 :: xratio(1:nx-1), yratio(1:ny-1), zratio(1:nz-1)
...
!   write(1,*) 'Moment of intertia = ', REAL(I_moment) ,' kg.m2 '
   ...
   write(1,*) '                '
	...
	do i=1,nx-1
		xx(1) = iDx(i)
		xx(2) = iDx(i+1)
		xratio(i) = maxval(xx)/minval(xx)*1.d0
	end do
	xMax = maxval(xratio)
	...
	do i=1,ny-1
		yy(1) = iDy(i)
		yy(2) = iDy(i+1)
		yratio(i) = maxval(yy)/minval(yy)*1.d0
	end do
	yMax = maxval(yratio)
	...
	do i=1,nz-1
		zz(1) = iDz(i)
		zz(2) = iDz(i+1)
		zratio(i) = maxval(zz)/minval(zz)*1.d0
	end do
	zMax = maxval(zratio)
	
   write(1,*) 'largest z ratio:',zMax
   write(1,*) 'largest y ratio:',yMax
...
!---------------------------------------- Pref = Pinf  =  pressure  at on inlet  ---------------------------------------
 !p_ref=0.d0
 !  do j=1,ny; do i=1,nx
     ! p_ref=p_ref+p(i,j,1)*iDx(i)*iDy(j)
  ! enddo; enddo
   !p_ref=p_ref/(lx*ly)

   !write(51,*) 'ZONE T = "p at <greek>h</greek>=0.5 , p<sub><math>%</math></sub> "'
   ...
   !do k=1,nz
     ...
     ! do j=1,ny
        ! if (ETA(1,j,k)>=0.5d0 .AND. ETA(1,j+1,k)<0.5d0) then
           ! x_c=DACOS(-2.d0*(Zs(k)-zc))*180.d0/PI                   ! theta  pressure loaction
           ! ratio=(ETA(1,j+1,k)-0.5d0)/(ETA(1,j+1,k)-ETA(1,j,k))    
           ! do i=1,nx
              ! p_avg=p_avg+p(i,j,k)*ratio + p(i,j+1,k)*(1.d0-ratio)  !  interpolated pressure
			   ! p_avg=p_avg+p(i,j,k)*ratio+p(i,j+1,k)*(1.d0-ratio)-p(i,1,1)*0.5d0-p(i,ny,1)*0.5d0
           ! enddo
      ...
      !enddo
   !enddo
!------------------------------------ Pressure refrence at inlet (1,1)-----------------------------------------------------
!------------------------------------ also calculate near boundary velocity paralel to surface-----------------------------
write(51,*) 'ZONE solutiontime=',  REAL(time)
      ...
      do j=1,ny
         if (ETA(1,j,k)>=0.5d0 .AND. ETA(1,j+1,k)<0.5d0) then
            ...
            do i=1,nx
			   ...
			   tan_vel_avg=tan_vel_avg+sqrt(w_mid**2*(SIN(x_c*PI/180.d0))**2+v_mid**2*(COS(x_c*PI/180.d0))**2)
			   dot = w_mid*(SIN(x_c*PI/180.d0))+v_mid*(COS(x_c*PI/180.d0))
			   if (dot<0.d0) then
					tan_vel_avg = tan_vel_avg*-1.d0
			   endif
            enddo
            p_avg=p_avg/(nx*1.d0)
