...
subroutine RayCasting() 
    ...
    do i = 1, poly
        ...
        az(i)= COS((-AOA)*PI/180.d0)*az(i) - SIN((-AOA)*PI/180.d0)*iay(i)     ! ROTATION
        ay(i)= SIN((-AOA)*PI/180.d0)*az(i) + COS((-AOA)*PI/180.d0)*iay(i) 
    ...
    do k=C,E;do j=A,B
            ...
            if ( intersection(k,j)>2 )then
                write(71,*)'!!!!!!!!!!!!!!!!!!!!!!!!!!!Raycasting wrong!!!!!!!!!!!!!!!!!!',k,j,REAL(AOA)
            endif
    ...
    !------------!
            ...
            if( points(k,j) > 0 )then
                    ...
                    !$OMP PARALLEL DO
                    do l = 1,nSubGrids+1
                        ZZ(l) = Z(k) + (l-1.d0) * ( Z(k+1)-Z(k) ) / DBLE(nSubGrids) !SUB_GRID SIZE IN X
                        YY(l) = Y(j) + (l-1.d0) * ( Y(j+1)-Y(j) ) / DBLE(nSubGrids) !SUB_GRID SIZE IN Y
                    enddo
                    !$OMP END PARALLEL DO
