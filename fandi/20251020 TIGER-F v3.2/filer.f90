...
implicit none
   ...
   write(11,*) ' ' 
   write(11,*) 'Last backup time = ',real(lastbackuptime),'sec'
   write(11,*) ' '    
...
implicit none
   ...
   !$OMP PARALLEL DO PRIVATE(j) collapse(1)
...
implicit none
   ...
   !$OMP PARALLEL DO PRIVATE(i,j) collapse(1)  
   !$acc parallel loop independent collapse(3) 
   do k=1,nz; do j=1,ny; do i=1,nx
      Qout(i,j,k,1) = p(i,j,k)
