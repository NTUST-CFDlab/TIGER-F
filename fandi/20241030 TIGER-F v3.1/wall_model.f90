...
real*8		:: KappaRe,yplusR,yplusR_,yplusB,uPlusB
   ...
   ! open (86,file='wall_viseff_instant.dat',position='append')
...
!$acc              v(:,:,istart-2:iend+2),w(:,:,istart-2:iend+2),Viseff(:,:,istart:iend))


! Calculate friction velocity uTau using the Newton-Raphson iteration method, calculate nut, modify Viseff
...
do k=istart,iend
	do j=jBgnVOS,jEndVOS
		...
		elseif(wdist(j,k) .LE. SQRT(max_dist)) then 					! Damping
			...
			uTau = SQRT((Viseff(1,j,k) + nu) * Urel/wdist(j,k))
			if(uTau .GT. ROOTVSMALL) then			! to avoid a division by zero, and wall function is not needed when Urel = 0
				...
				if(wall_model == 1 .AND. wdist(j,k) .LE. SQRT(dzSml**2+dySml**2)) then			! Applied only on the nearest cells
					Viseff(1,j,k) = DMAX1(0.d0,(uTau*uTau/(Urel/wdist(j,k) + ROOTVSMALL)) - nu)
				elseif(Damping_F == 1 .AND. ypluss(j,k) .LT. 300.d0) then
					fdamp(j,k) = 1.d0 - dexp(-ypluss(j,k)/25.d0)
					Viseff(1,j,k) = fdamp(j,k)**2*Viseff(1,j,k)				! Damping
				endif
...
if (istep == INT(100.d0/dt)) then     
	!$acc update self(wdist,fdamp(:,istart:iend),ypluss(:,istart:iend),upluss(:,istart:iend),Viseff(:,:,istart:iend))
      ...
      if(myid>master)then
         ...
         itag = 14
         call MPI_SEND( Viseff(-1,-1,istart), igcount*(nx+4)*(ny+4), MPI_REAL8, master, itag, MPI_COMM_WORLD, ierr )
      end if
      ...
      if(myid==master)then
         do i = 1, (nproc-1)
            ...
            itag = 14
            call MPI_RECV( Viseff(-1,-1,gstart(i)), igcount*(nx+4)*(ny+4), MPI_REAL8, i, itag, MPI_COMM_WORLD, status, ierr )
         end do
      end if
      ...
      if(myid==master)then
		do k=kBgnVOS,kEndVOS
			do j=jBgnVOS,jEndVOS
				...
				write(85,'(5(3x,F15.10))') Ys(j),Zs(k),wdist(j,k),ypluss(j,k),Viseff(1,j,k)
				write(86,'(3(3x,F15.10))') Ys(j),Zs(k),Viseff(1,j,k)
