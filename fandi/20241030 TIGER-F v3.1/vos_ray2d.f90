...
subroutine vos_ray2d() 		! CPU cores only!
    use variables
    implicit none
    integer 					:: l ,m ,n, off
...
!$acc data present(Z,Y,Zs,Ys,ETA) copyin(iaz,iay) create(az,ay,points,intersection,ETA_1,sub_intersection)
    ...
    do n = 1, poly
        ...
        az(n)= DCOS((AOA)*PI/180.d0)*iaz(n) - DSIN((AOA)*PI/180.d0)*iay(n)     ! ROTATION 
        ay(n)= DSIN((AOA)*PI/180.d0)*iaz(n) + DCOS((AOA)*PI/180.d0)*iay(n) 
    ...
    !--------------!
            ...
            if( (ay(m) >= Ys(j)) .AND. (ay(m+1) < Ys(j)))then            ! Xs, Ys, Zs : Midpoints of grid coordinates
                !-------------------------!
                    ...
                    do k=C,E
                        if( Zs(k) > DMAX1( az(m),az(m+1) ) )then
    ...
    !------------!
            ...
            if( points(k,j) > 0 )then
                    ...
                    do l = 1,nSubGrids_2d+1
                        ZZ(l) = Z(k) + (l-1.d0) * ( Z(k+1)-Z(k) ) / (nSubGrids_2d*1.d0) !SUB_GRID SIZE IN X
                        YY(l) = Y(j) + (l-1.d0) * ( Y(j+1)-Y(j) ) / (nSubGrids_2d*1.d0) !SUB_GRID SIZE IN Y
                    enddo
                    !$OMP END PARALLEL DO
                    ...
                    !-------------------!
                        ...
                        if( (ay(m) >=  (YY(l)+YY(l+1))/2.d0) .AND. (ay(m+1) <= (YY(l)+YY(l+1))/2.d0) )then
                                !-------------------------!
								do n=1, nSubGrids_2d
                                    if( (ZZ(n)+ZZ(n+1))/2.d0 > DMAX1(az(m),az(m+1)) )then
                ...
                ETA_1(k,j) = 1.d0*total/(nSubGrids_2d*1.d0)/(nSubGrids_2d*1.d0)
...
! subroutine vos_ray2d() 		GPU offloading in progress (error in moving solid case)
    ! use variables
    ! implicit none
    ! integer 			:: l ,m ,n
    ! real*8,dimension(1:poly)	:: az, ay
...
! !$acc update self(az,ay)	 
    ...
    ! !$OMP PARALLEL DO PRIVATE(j)
	! !$acc parallel 
	! !$acc loop independent private(j) gang vector
    ! do m=1,poly-1  !
        ! if( ((ay(m+1)-ay(m))**2 + (az(m+1)-az(m))**2) < min_dist**2) then
