...
do k=1,nz
	...
	z_aoa1 =  Zf*dcos((AOA-90.d0)*PI/180.d0)+Yf*dsin((AOA-90.d0)*PI/180.d0)
	y_aoa1 = -Zf*dsin((AOA-90.d0)*PI/180.d0)+Yf*dcos((AOA-90.d0)*PI/180.d0)
	...
	z_aoa2 =  Zf*dcos((AOA-90.d0+120.d0)*PI/180.d0)+Yf*dsin((AOA-90.d0+120.d0)*PI/180.d0)
	y_aoa2 = -Zf*dsin((AOA-90.d0+120.d0)*PI/180.d0)+Yf*dcos((AOA-90.d0+120.d0)*PI/180.d0)
	...
	z_aoa3 =  Zf*dcos((AOA-90.d0+240.d0)*PI/180.d0)+Yf*dsin((AOA-90.d0+240.d0)*PI/180.d0)
	y_aoa3 = -Zf*dsin((AOA-90.d0+240.d0)*PI/180.d0)+Yf*dcos((AOA-90.d0+240.d0)*PI/180.d0)
...
! Creating subgrids
i=iBgnVOS
!$OMP PARALLEL DO PRIVATE(j,m,n,Yf,Zf,SY,SZ,xi,z_aoa1,y_aoa1,z_trans1,y_trans1,z_aoa2,y_aoa2,z_trans2,y_trans2,z_aoa3,y_aoa3,z_trans3,y_trans3) collapse(2)
!$acc parallel vector_length(32)
!$acc loop private(m,n,Yf,Zf,SY,SZ,xi,z_aoa1,y_aoa1,z_trans1,y_trans1,z_aoa2,y_aoa2,z_trans2,y_trans2,z_aoa3,y_aoa3,z_trans3,y_trans3) collapse(2) gang
...
do j=jBgnVOS,jEndVOS
	...
	if(ANY(ETA(i,j,k) /= ETA(i, j-1:j+1, k-1:k+1))) then
		    ...
		    xi = 0
            !$acc loop private(n,Yf,Zf,z_aoa1,y_aoa1,z_trans1,y_trans1,z_aoa2,y_aoa2,z_trans2,y_trans2,z_aoa3,y_aoa3,z_trans3,y_trans3) reduction(+:xi)
            do m=1,nSubGrids_f,1
            !$acc loop private(Yf,Zf,z_aoa1,y_aoa1,z_trans1,y_trans1,z_aoa2,y_aoa2,z_trans2,y_trans2,z_aoa3,y_aoa3,z_trans3,y_trans3) reduction(+:xi)
            do n=1,nSubGrids_f,1
...
do k=1,nz
	...
	! rotation wrt. AOA for blade 1
	z_aoa1 =  Zf*dcos((AOA)*PI/180.d0)+Yf*dsin((AOA)*PI/180.d0)
	y_aoa1 = -Zf*dsin((AOA)*PI/180.d0)+Yf*dcos((AOA)*PI/180.d0)
	...
	! rotation wrt. AOA for blade 2
	z_aoa2 =  Zf*dcos((AOA+120.d0)*PI/180.d0)+Yf*dsin((AOA+120.d0)*PI/180.d0)
	y_aoa2 = -Zf*dsin((AOA+120.d0)*PI/180.d0)+Yf*dcos((AOA+120.d0)*PI/180.d0)
	...
	! rotation wrt. AOA for blade 3
	z_aoa3 =  Zf*dcos((AOA+240.d0)*PI/180.d0)+Yf*dsin((AOA+240.d0)*PI/180.d0)
	y_aoa3 = -Zf*dsin((AOA+240.d0)*PI/180.d0)+Yf*dcos((AOA+240.d0)*PI/180.d0)

	! z and y transformation for flat plate
	z_trans3 = (z_aoa3 - rotor_r)           + r*dsin(S_p)
	y_trans3 = (y_aoa3 + (g_p+r)+0.5d0*L_p) - r*(1.d0 - dcos(S_p)) + 0.5d0*t_p*dsin(S_p)


	if( (((z_aoa1-rotor_r)**2 + y_aoa1**2) .LE. r**2 .OR. (dabs(z_trans1/t_p + y_trans1/L_p) + dabs(z_trans1/t_p - y_trans1/L_p)) .LE. 1) .OR. &
	    (((z_aoa2-rotor_r)**2 + y_aoa2**2) .LE. r**2 .OR. (dabs(z_trans2/t_p + y_trans2/L_p) + dabs(z_trans2/t_p - y_trans2/L_p)) .LE. 1) .OR. &
	    (((z_aoa3-rotor_r)**2 + y_aoa3**2) .LE. r**2 .OR. (dabs(z_trans3/t_p + y_trans3/L_p) + dabs(z_trans3/t_p - y_trans3/L_p)) .LE. 1)) then
		ETA(i,j,k)=1.D0
		az_min = MIN0(k,az_min)
		az_max = MAX0(k,az_max)
		ay_min = MIN0(j,ay_min)
...
do k=1,nz
	...
	! rotation wrt. AOA for blade 2
	z_aoa2 =  Zf*dcos((AOA+180.d0)*PI/180.d0)+Yf*dsin((AOA+180.d0)*PI/180.d0)
	y_aoa2 = -Zf*dsin((AOA+180.d0)*PI/180.d0)+Yf*dcos((AOA+180.d0)*PI/180.d0)
...
! Creating subgrids
i=iBgnVOS
!$OMP PARALLEL DO PRIVATE(j,m,n,Yf,Zf,SY,SZ,xi,z_aoa1,y_aoa1,z_trans1,y_trans1,z_aoa2,y_aoa2,z_trans2,y_trans2) collapse(2)
!$acc parallel vector_length(32)
!$acc loop private(m,n,Yf,Zf,SY,SZ,xi,z_aoa1,y_aoa1,z_trans1,y_trans1,z_aoa2,y_aoa2,z_trans2,y_trans2) collapse(2) gang
...
do j=jBgnVOS,jEndVOS
	...
	if(ANY(ETA(i,j,k) /= ETA(i, j-1:j+1, k-1:k+1))) then
		    ...
		    xi = 0
            !$acc loop private(n,Yf,Zf,z_aoa1,y_aoa1,z_trans1,y_trans1,z_aoa2,y_aoa2,z_trans2,y_trans2) reduction(+:xi)
            do m=1,nSubGrids_f,1
            !$acc loop private(Yf,Zf,z_aoa1,y_aoa1,z_trans1,y_trans1,z_aoa2,y_aoa2,z_trans2,y_trans2) reduction(+:xi)
            do n=1,nSubGrids_f,1
...
! Creating subgrids
i=iBgnVOS
!$OMP PARALLEL DO PRIVATE(j,m,n,Yf,Zf,SY,SZ,xi,z_aoa1,y_aoa1,z_trans1,y_trans1) collapse(2)
!$acc parallel vector_length(32)
!$acc loop private(m,n,Yf,Zf,SY,SZ,xi,z_aoa1,y_aoa1,z_trans1,y_trans1) collapse(2) gang
...
do j=jBgnVOS,jEndVOS
	...
	if(ANY(ETA(i,j,k) /= ETA(i, j-1:j+1, k-1:k+1))) then
		    ...
		    xi = 0
            !$acc loop private(n,Yf,Zf,z_aoa1,y_aoa1,z_trans1,y_trans1) reduction(+:xi)
            do m=1,nSubGrids_f,1
            !$acc loop private(Yf,Zf,z_aoa1,y_aoa1,z_trans1,y_trans1) reduction(+:xi)
            do n=1,nSubGrids_f,1
...
! Creating subgrids
i=iBgnVOS
!$OMP PARALLEL DO PRIVATE(j,m,n,Yf,Zf,SY,SZ,xi,z_aoa1,y_aoa1) collapse(2)
!$acc parallel vector_length(32)
!$acc loop private(m,n,Yf,Zf,SY,SZ,xi,z_aoa1,y_aoa1) collapse(2) gang
...
do j=jBgnVOS,jEndVOS
	...
	if(ANY(ETA(i,j,k) /= ETA(i, j-1:j+1, k-1:k+1))) then
		    ...
		    xi = 0
            !$acc loop private(n,z_aoa1,y_aoa1) reduction(+:xi)
            do m=1,nSubGrids_f,1
            !$acc loop private(z_aoa1,y_aoa1) reduction(+:xi)
            do n=1,nSubGrids_f,1
...
!$OMP PARALLEL DO PRIVATE(j,Yf,Zf,z_aoa1,y_aoa1,z_trans1) &
!$OMP			  REDUCTION(max : az_max,ay_max) REDUCTION(min : az_min,ay_min) collapse(2)
!$acc parallel vector_length(32)
!$acc loop private(Yf,Zf,z_aoa1,y_aoa1,z_trans1) &
!$acc 	   reduction(max : az_max,ay_max) reduction(min : az_min,ay_min) collapse(2) gang vector
...
do j=jBgnVOS,jEndVOS
	...
	if(ANY(ETA(i,j,k) /= ETA(i, j-1:j+1, k-1:k+1))) then
		    ...
		    xi = 0
            !$acc loop private(n,Yf,Zf,z_aoa1,y_aoa1,z_trans1) reduction(+:xi)
            do m=1,nSubGrids_f,1
            !$acc loop private(Yf,Zf,z_aoa1,y_aoa1,z_trans1) reduction(+:xi)
            do n=1,nSubGrids_f,1
...
do k=1,nz
	...
	z_aoa1 =  Zf * dcos((AOA)*PI/180.d0) + Yf * dsin((AOA)*PI/180.d0) + offset_z * (1 - dcos((AOA)*PI/180.d0))
	y_aoa1 = -Zf * dsin((AOA)*PI/180.d0) + Yf * dcos((AOA)*PI/180.d0) - offset_z * dsin((AOA)*PI/180.d0)
...
do j=jBgnVOS,jEndVOS
	...
	if(ANY(ETA(i,j,k) /= ETA(i, j-1:j+1, k-1:k+1))) then
		    ...
		    xi = 0.0
            !$acc loop private(n,Yf,Zf,z_aoa1,y_aoa1) reduction(+:xi)
            do m=1,nSubGrids_f,1
            !$acc loop private(Yf,Zf,z_aoa1,y_aoa1) reduction(+:xi)
            do n=1,nSubGrids_f,1
...
! ! Creating subgrids
! i=iBgnVOS
! !$OMP PARALLEL DO PRIVATE(j,m,n,Yf,Zf,SY,SZ,xi,z_aoa1,y_aoa1,z_trans_,y_trans_,z_trans1,y_trans1,z_aoa2,y_aoa2,z_trans2,y_trans2) collapse(2)
! !$acc parallel vector_length(32)
! !$acc loop private(m,n,Yf,Zf,SY,SZ,xi,z_aoa1,y_aoa1,z_trans_,y_trans_,z_trans1,y_trans1,z_aoa2,y_aoa2,z_trans2,y_trans2) collapse(2) gang
...
! do j=jBgnVOS,jEndVOS
	...
	! if(ANY(ETA(i,j,k) /= ETA(i, j-1:j+1, k-1:k+1))) then
		    ...
		    ! xi = 0
            ! !$acc loop private(n,Yf,Zf,z_aoa1,y_aoa1,z_trans_,y_trans_,z_trans1,y_trans1,z_aoa2,y_aoa2,z_trans2,y_trans2) reduction(+:xi)
            ! do m=1,nSubGrids_f,1
            ! !$acc loop private(Yf,Zf,z_aoa1,y_aoa1,z_trans_,y_trans_,z_trans1,y_trans1,z_aoa2,y_aoa2,z_trans2,y_trans2) reduction(+:xi)
            ! do n=1,nSubGrids_f,1
...
! do k=1,nz
	...
	! if( (((z_aoa1+rotor_r)**2 + y_aoa1**2) .LE. r**2) .OR. ((dabs(z_trans1/t_p + y_trans1/L_p) + dabs(z_trans1/t_p - y_trans1/L_p)) .LE. 1)) then
		! ETA(i,j,k)=1.D0
		! az_min = MIN0(k,az_min)
		! az_max = MAX0(k,az_max)
		! ay_min = MIN0(j,ay_min)
...
! ! Creating subgrids
! i=iBgnVOS
! !$OMP PARALLEL DO PRIVATE(j,m,n,Yf,Zf,SY,SZ,xi,z_aoa1,y_aoa1,z_trans_,y_trans_,z_trans1,y_trans1) collapse(2)
! !$acc parallel vector_length(32)
! !$acc loop private(m,n,Yf,Zf,SY,SZ,xi,z_aoa1,y_aoa1,z_trans_,y_trans_,z_trans1,y_trans1) collapse(2) gang
...
! do j=jBgnVOS,jEndVOS
	...
	! if(ANY(ETA(i,j,k) /= ETA(i, j-1:j+1, k-1:k+1))) then
		    ...
		    ! xi = 0
            ! !$acc loop private(n,Yf,Zf,z_aoa1,y_aoa1,z_trans_,y_trans_,z_trans1,y_trans1) reduction(+:xi)
            ! do m=1,nSubGrids_f,1
            ! !$acc loop private(Yf,Zf,z_aoa1,y_aoa1,z_trans_,y_trans_,z_trans1,y_trans1) reduction(+:xi)
            ! do n=1,nSubGrids_f,1
...
do k=1,nz
	...
	if( (dabs(z_trans1/t_p + y_trans1/L_p) + dabs(z_trans1/t_p - y_trans1/L_p)) .LE. 1) then
		ETA(i,j,k)=1.D0
		az_min = MIN0(k,az_min)
		az_max = MAX0(k,az_max)
		ay_min = MIN0(j,ay_min)
...
!Creating subgrids
i=iBgnVOS
!$OMP PARALLEL DO PRIVATE(j,m,n,SY,SZ,xi) collapse(2)
!$acc parallel vector_length(32)
!$acc loop private(m,n,SY,SZ,xi) collapse(2) gang
...
!$acc data present(X,Y,Z,Xs,Ys,Zs,iDx,iDy,iDz,ETA) create(SX,SY,SZ)
...
do k=1,nz
	...
	if( Zf**2 + Yf**2 + Xf**2 .LE. r**2) then				! Geometry function
		...
		ay_min = MIN0(j,ay_min)
		ay_max = MAX0(j,ay_max)
		ax_min = MIN0(i,ax_min)
		ax_max = MAX0(i,ax_max)
	else
		ETA(i,j,k)=0.D0		
	endif
		end do
...
!$acc data present(X,Y,Z,Xs,Ys,Zs,iDx,iDy,iDz,ETA) create(SX,SY,SZ,x0_,y0_,z0_,sphere)
