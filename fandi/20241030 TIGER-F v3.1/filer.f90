...
!   end if
   ...
   write(1,*) ' '
      ...
      if(Gridder=='non-uniform-fall')then 
   write(1,*) '=========================ground-3D==========================='
   write(1,*) 'Small dx = ',REAL(dxSml)
   write(1,*) 'Small dy = ',REAL(dySml)
   write(1,*) 'Small dz = ',REAL(dzSml)
...
!   write(1,*) 'velocity Residual =',REAL(zeta_vel)
   ...
   write(31,*) ' TITLE     = "" '
   write(31,*) ' VARIABLES = t*,AOA,C<sub>T</sub>,C<sub>P</sub>'
...
implicit none
   ...
   write(31,'(F12.7,3X,F14.6,2(3X,F12.7))') REAL(time), REAL(AOA), REAL(cTorq), REAL(cPower)
    ...
    do k=nz,1,-1											! for 0<theta<180 (upper half)
      ...
      do j=1,ny
         if (ETA(1,j,k)>=0.5d0 .AND. ETA(1,j+1,k)<0.5d0) then
            ...
            do i=1,nx
               p_avg=p_avg+p(i,j+1,k)*ratio+p(i,j,k)*(1.d0-ratio)-p(i,1,1)
                        w_mid=w(i,j+2,k)*ratio+w(i,j+1,k)*(1.d0-ratio)
                        v_mid=v(i,j+2,k)*ratio+v(i,j+1,k)*(1.d0-ratio)   
                        tan_vel_avg=tan_vel_avg+(-w_mid*SIN(x_c*PI/180.d0)+v_mid*COS(x_c*PI/180.d0))
            enddo
            p_avg=p_avg/(nx*1.d0)
            tan_vel_avg=tan_vel_avg/(nx*1.d0)
...
subroutine filereachtime2d() ! use this for 2D spanwise (Z axis) averaged
use variables
implicit none
...
! subroutine filereachtime2d()	! use this for 2D slice at the middle of Z plane
! use variables
! implicit none
...
!real*8 ,dimension(1:nx,1:ny,1:nz)  :: TKE_u, TKE_v, TKE_w, u_filter_avg, v_filter_avg, w_filter_avg
	...
	write(filename,'(A,I4.4)')'Avg_',NINT(time/isto3d_int)
	fileformat = '.q'
...
implicit none
   ...
   write(18) time
   write(18) u_solid,v_solid,w_solid,rotor_omega,AOA
   write(18) x0_t,y0_t,z0_t
