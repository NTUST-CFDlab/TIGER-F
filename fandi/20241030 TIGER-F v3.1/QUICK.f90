...
!$acc data present(iDx,iDy,iDz,Dxs,Dys,Dzs,Viseff(:,:,istart:iend), &
...
!$acc	u_star(:,:,istart-2:iend+2),v_star(:,:,istart-2:iend+2),w_star(:,:,istart-2:iend+2))
  ...
  !$OMP PARALLEL
   !$OMP DO PRIVATE(i,j,u_tilde_x1,u_tilde_x2,v_tilde_x1,v_tilde_x2,w_tilde_x1,w_tilde_x2,ue,uw,un,us,uf,ub,nut) collapse(nclps)
   !$acc parallel
   !$acc loop independent private(u_tilde_x1,u_tilde_x2,v_tilde_x1,v_tilde_x2,w_tilde_x1,w_tilde_x2,ue,uw,un,us,uf,ub,nut) collapse(3) gang vector
   do k=istart,iend
   do j=1,ny; do i=1,nx
      ...
      w_tilde_x2 = 0.5d0 * (w(i,j,k-1)+w(i+1,j,k-1))
        ...
        u_star(i,j,k) = -dt*(u_tilde_x1*ue-u_tilde_x2*uw) / Dxs(i) &
						-dt*(v_tilde_x1*un-v_tilde_x2*us) / iDy(j) &
						-dt*(w_tilde_x1*uf-w_tilde_x2*ub) / iDz(k) &

                        +(1.d0*nu+nut*LES)*dt*( (u(i+1,j,k)-u(i,j,k)) / iDx(i+1) - (u(i,j,k)-u(i-1,j,k)) / iDx(i) ) / Dxs(i) &
                        +(1.d0*nu+nut*LES)*dt*( (u(i,j+1,k)-u(i,j,k)) / Dys(j) - (u(i,j,k)-u(i,j-1,k)) / Dys(j-1) ) / iDy(j) &
                        +(1.d0*nu+nut*LES)*dt*( (u(i,j,k+1)-u(i,j,k)) / Dzs(k) - (u(i,j,k)-u(i,j,k-1)) / Dzs(k-1) ) / iDz(k) 
...
!!! v_star calculation (y component) !!!
   ...
   !$OMP DO PRIVATE(i,j,u_tilde_y1,u_tilde_y2,v_tilde_y1,v_tilde_y2,w_tilde_y1,w_tilde_y2,ve,vw,vn,vs,vf,vb,nut) collapse(nclps)
   !$acc loop independent private(u_tilde_y1,u_tilde_y2,v_tilde_y1,v_tilde_y2,w_tilde_y1,w_tilde_y2,ve,vw,vn,vs,vf,vb,nut) collapse(3) gang vector
   do k=istart,iend
   do j=1,ny;do i=1,nx
      ...
      w_tilde_y2 = 0.5d0 * (w(i,j,k-1)+w(i,j+1,k-1))
        ...
        v_star(i,j,k) = -dt*(u_tilde_y1*ve-u_tilde_y2*vw) / iDx(i) &
                        -dt*(v_tilde_y1*vn-v_tilde_y2*vs) / Dys(j) &
                        -dt*(w_tilde_y1*vf-w_tilde_y2*vb) / iDz(k) &

                        +(1.d0*nu+nut*LES)*dt*( (v(i+1,j,k)-v(i,j,k)) / Dxs(i) - (v(i,j,k) - v(i-1,j,k)) / Dxs(i-1) ) / iDx(i) &
                        +(1.d0*nu+nut*LES)*dt*( (v(i,j+1,k)-v(i,j,k)) / iDy(j+1) - (v(i,j,k) - v(i,j-1,k)) / iDy(j) ) / Dys(j) &
                        +(1.d0*nu+nut*LES)*dt*( (v(i,j,k+1)-v(i,j,k)) / Dzs(k) - (v(i,j,k) - v(i,j,k-1)) / Dzs(k-1) ) / iDz(k) 
                        !+dt*0.5d0*( F_tavey(i,j,k)+F_tavey(i,j+1,k) )*unDBD_onoff
                        !-dt*( (1.d0*nu+nut*LES)*ABS(F_tavey(i,j,k)) )
...
!!! w_star calculation (z component) !!!
   ...
   !$OMP DO PRIVATE(i,j,u_tilde_z1,u_tilde_z2,v_tilde_z1,v_tilde_z2,w_tilde_z1,w_tilde_z2,we,ww,wn,ws,wf,wb,nut) collapse(nclps)
   !$acc loop independent private(u_tilde_z1,u_tilde_z2,v_tilde_z1,v_tilde_z2,w_tilde_z1,w_tilde_z2,we,ww,wn,ws,wf,wb,nut) collapse(3) gang vector
   do k=istart,iend 
   do j=1,ny;do i=1,nx
        ...
        w_star(i,j,k) = -dt*(u_tilde_z1*we-u_tilde_z2*ww) / iDx(i) &
                        -dt*(v_tilde_z1*wn-v_tilde_z2*ws) / iDy(j) &
                        -dt*(w_tilde_z1*wf-w_tilde_z2*wb) / Dzs(k) &
                                
                        +(1.d0*nu+nut*LES)*dt*( ( w(i+1,j,k)-w(i,j,k) ) / Dxs(i) -  ( w(i,j,k)-w(i-1,j,k) ) / Dxs(i-1)  ) / iDx(i) &
                        +(1.d0*nu+nut*LES)*dt*( ( w(i,j+1,k)-w(i,j,k) ) / Dys(j) -  ( w(i,j,k)-w(i,j-1,k) ) / Dys(j-1)  ) / iDy(j) &
                        +(1.d0*nu+nut*LES)*dt*( ( w(i,j,k+1)-w(i,j,k) ) / iDz(k+1) - ( w(i,j,k)-w(i,j,k-1) ) / iDz(k)   ) / Dzs(k) 
                        !+ dt*0.5d0*( F_tavex(i,j,k)+F_tavex(i,j,k+1) )*unDBD_onoff
                        !+ dt*( (1.d0*nu+nut*LES)*ABS(F_tavex(i,j,k)) )
  end do;end do
  end do
  !$acc end parallel
  !$OMP END DO
