...
!$acc data create(nk(jbgnvos:jendvos,istart_vos:iend_vos),n_(jbgnvos:jendvos,istart_vos:iend_vos,1:nf))
    ...
    do j=jBgnVOS,jEndVOS
		...
		!!$acc loop seq
		do n=1,nf
			if( ( AMAX1(Pz((n-1)*3+1),Pz((n-1)*3+2),Pz((n-1)*3+3)) .GE. Zs(k)-iDz(k)/2.) .AND. (AMIN1(Pz((n-1)*3+1),Pz((n-1)*3+2),Pz((n-1)*3+3)) .LE. Zs(k)+iDz(k)/2.) .AND. &
				( AMAX1(Py((n-1)*3+1),Py((n-1)*3+2),Py((n-1)*3+3)) .GE. Ys(j)-iDy(j)/2.) .AND. (AMIN1(Py((n-1)*3+1),Py((n-1)*3+2),Py((n-1)*3+3)) .LE. Ys(j)+iDy(j)/2.) ) then
				nn = nn+1
				n_(j,k,nn) = n
			endif
		enddo
...
! !        !$OMP PARALLEL DO PRIVATE(c1,c2,c3) SHARED(nc,ray_inter)
        ...
        ! do n=1,nk(j,k)                                                     ! Only facets with interiors intersected by the ray are included.			 
				! nn = n_(j,k,n)
                ! if ( (Zsub .GE. AMIN1(Pz((nn-1)*3+1),Pz((nn-1)*3+2),Pz((nn-1)*3+3)) .AND. Zsub .LE. AMAX1(Pz((nn-1)*3+1),Pz((nn-1)*3+2),Pz((nn-1)*3+3))) .AND.  &
					 ! (Ysub .GE. AMIN1(Py((nn-1)*3+1),Py((nn-1)*3+2),Py((nn-1)*3+3)) .AND. Ysub .LE. AMAX1(Py((nn-1)*3+1),Py((nn-1)*3+2),Py((nn-1)*3+3))) ) then
...
! !        !$OMP PARALLEL DO PRIVATE(c1,c2,c3) SHARED(nc,ray_inter)
		! !$acc loop seq
        ! do n=1,nk(j,k)                                                     ! Only facets with interiors intersected by the ray are included.			 
                ! if ( (Zsub .GE. AMIN1(Pz((n_(j,k,n)-1)*3+1),Pz((n_(j,k,n)-1)*3+2),Pz((n_(j,k,n)-1)*3+3)) .AND. Zsub .LE. AMAX1(Pz((n_(j,k,n)-1)*3+1),Pz((n_(j,k,n)-1)*3+2),Pz((n_(j,k,n)-1)*3+3))) .AND.  &
					 ! (Ysub .GE. AMIN1(Py((n_(j,k,n)-1)*3+1),Py((n_(j,k,n)-1)*3+2),Py((n_(j,k,n)-1)*3+3)) .AND. Ysub .LE. AMAX1(Py((n_(j,k,n)-1)*3+1),Py((n_(j,k,n)-1)*3+2),Py((n_(j,k,n)-1)*3+3))) ) then
                        ! ! Intersection test using the line and point equation (cross product) (Position of a Point Relative to a Line or side of triangle)
...
! !        !$OMP PARALLEL DO PRIVATE(c1,c2,c3) SHARED(nc,ray_inter)	
        ! do n=1,nf                                                     ! Only facets with centroid position within the possible range in z & y are included
                ! if ( (Zsub .GE. AMIN1(Pz((n-1)*3+1),Pz((n-1)*3+2),Pz((n-1)*3+3)) .AND. Zsub .LE. AMAX1(Pz((n-1)*3+1),Pz((n-1)*3+2),Pz((n-1)*3+3))) .AND.  &
					 ! (Ysub .GE. AMIN1(Py((n-1)*3+1),Py((n-1)*3+2),Py((n-1)*3+3)) .AND. Ysub .LE. AMAX1(Py((n-1)*3+1),Py((n-1)*3+2),Py((n-1)*3+3))) ) then
                        ! ! Intersection test using the line and point equation (cross product) (Position of a Point Relative to a Line or side of triangle)
