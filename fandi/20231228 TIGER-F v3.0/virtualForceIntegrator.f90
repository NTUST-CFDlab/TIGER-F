...
!$acc data present(iDx,iDy,iDz,Xs,Ys,Zs,FX(:,:,istart-2:iend+2),FY(:,:,istart-2:iend+2),FZ(:,:,istart-2:iend+2))
	...
	do j=1,ny;do i=1,nx
		...
		totalT_XZ = totalT_XZ + den_flu*( FZ(i,j,k)*(ys(j)-yc_t)*(iDx(i)*iDy(j)*iDz(k) ) )        !CCW > 0    
		totalT_XY = totalT_XY + den_flu*(-FY(i,j,k)*(zs(k)-zc_t)*(iDx(i)*iDy(j)*iDz(k) ) )        !CCW > 0
		
		
		totalT_YZ = totalT_YZ + den_flu*(-FZ(i,j,k)*(xs(i)-xc_t)*(iDx(i)*iDy(j)*iDz(k) ) )        !CCW > 0    
		totalT_YX = totalT_YX + den_flu*( FX(i,j,k)*(zs(k)-zc_t)*(iDx(i)*iDy(j)*iDz(k) ) )        !CCW > 0
		
		
		totalT_ZY = totalT_ZY + den_flu*( FY(i,j,k)*(xs(i)-xc_t)*(iDx(i)*iDy(j)*iDz(k) ) )        !CCW > 0    
		totalT_ZX = totalT_ZX + den_flu*(-FX(i,j,k)*(ys(j)-yc_t)*(iDx(i)*iDy(j)*iDz(k) ) )        !CCW > 0
