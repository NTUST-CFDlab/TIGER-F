...
implicit none
   ...
   write(31,'(F12.7,3X,F14.6,2(3X,F12.7))') REAL(time), REAL(-1.*AOA), REAL(cTorq), REAL(cPower)
...
implicit none
   ...
   open (19,file=TRIM('output2D/')//TRIM(filename)//fileformat,form='unformatted')
   write(19) nblocks
   write(19) 1, ny, nz
...
implicit none
   ...
   if( time < StartDynamic_time+dt )then
      open (unit=18,form='unformatted',file='Static0.bak')
   else 
      open (unit=18,form='unformatted',file='OOXX.bak')
   endif
   write(18) nblocks
   write(18) nx, ny, nz
