...
!----------------Unequal grid intervals----------------!
    ...
    do j=1,ny+1
        ...
        elseif( j > nyLrg1+nyMid+1 .AND. j <= ny+1 ) then
    ...
    do k=1,nz+1
        ...
        if ( k==1 ) then
            Z(k) = 0.d0
        elseif( k <= nzLrg1 ) then
        
            thetaa = 0.5d0*PI*(k-1.d0)/nzLrg1*1.d0 
            Z(k) = (zLrg1-dzSml*nzLrg1)*sin(thetaa) + dzSml*(k-1.d0)

        elseif( k > nzLrg1 .AND. k <= nzLrg1+nzSml+1 ) then
            Z(k) = zLrg1 + dzSml*(k-nzLrg1-1.d0)

        elseif( k > nzLrg1+nzSml+1 .AND. k <= nzLrg1+nzMid+1 ) then
        ...
        elseif( k > nzLrg1+nzMid+1 .AND. k <= nz+1 ) then
