...
program main
   ...
   LES_mode                         = 'ON'                	! the LES mode 'WALE' = WALE model, 'SMAG' = Smagorinsky model, 'OFF' = DNS 

   wall_model						= 'OFF'					! Apply wall model 'ON' or 'OFF'	!!!under development, only for infinite cylinder currently!!!

   heat_transfer					= 'OFF'					! Heat transfer 'CONJ' = Conjugate HT, 'ISOT' = Isothermal, 'OFF' = No heat transfer 
   ...
   VOS_by							= 1						! Create VOS by 1)geometry function, 2)raycasting2D, 3)raycasting3D, 0)no VOS
...
!															  SOR with a dynamic omega is used. Set omega bounds in SOR.f90 when necessary. (default = 1.1-1.9)
   ...
   time_stepping_by					= 'CFL'					! 'DT' = constant dt, 'CFL' = constant CFL
					...
					constant_cfl	= 0.1d0					! input cfl if time_stepping_by a constant cfl (use 0.2 if using 2nd/3rd order AB)
   ...
   filer2d							= 'ON'					! create 2D output (2d_xxxx.q). 'MID'/'ON' : Spanwise Midplane slice ; 'AVG' : Spanwise avg
   ...
   filer_virtual_force				= 'OFF'					! create 3D output of virtual force field.
			...
			filerVF_int				= 100.d0   				! time interval to write virtual force files
   ...
   filer_near_wall					= 'OFF'					! create 2D midplane output of near-wall properties (y,y+,u+,nut).
			startfilerNW_time		= 0.d0					! initialize time of writing near-wall properties files
			filerNW_int				= 1.d0   				! time interval to write near-wall properties files
...
!      call gridder_sin()              ! define unequal mesh
  ! else if(Gridder=='read_txt' )then
   ...
   if (resume=='OFF') then
	  ...
	  if(heat_transfer/='OFF') call read_backupfile_HT() 
   ...
   !--------------------- Intervals handler ------------------------!
	istep = 0
	dt_1 = dt ; dt_2 = dt							! for Adams Bashforth
	...
	next_filerNW  = MAX(INT(initial_time/filerNW_int) *filerNW_int  + filerNW_int , startfilerNW_time)
...
!$acc end data
	...
	if (heat_transfer/='OFF') then
		!$acc data copy(theta(:,:,istart-2:iend+2))
		!----------------------------------------------------------------!	*** boundary_conditions.f90 ***
		call theta_boundary_conditions()       !call boundary conditions
		!----------------------------------------------------------------!
		!$acc end data
	endif
		...
		! solid_volume_1 = 0.0		! For tandem cylinders
		! solid_volume_2 = 0.0
		! do k=1,nz; do j=1,ny; do i=1,nx
			! if(Zs(k) .LT. Zsplit_pos) then
				! solid_volume_1=solid_volume_1 + ETA(i,j,k)*iDx(i)*iDy(j)*iDz(k)
			! else
				! solid_volume_2=solid_volume_2 + ETA(i,j,k)*iDx(i)*iDy(j)*iDz(k)
			! endif
		! enddo; enddo; enddo
...
!$acc	  P_Den(:,:,istart:iend,:),  Den_inv(:,:,istart:iend),     	  nut(:,:,istart-2:iend+2)  , &
!$acc	     u0(:,:,istart-2:iend+2),     v0(:,:,istart-2:iend+2),     w0(:,:,istart-2:iend+2), &
!$acc	      u(:,:,istart-2:iend+2),      v(:,:,istart-2:iend+2),      w(:,:,istart-2:iend+2)) &
...
!$acc		 FX(:,:,istart-2:iend+2),     FY(:,:,istart-2:iend+2),     FZ(:,:,istart-2:iend+2),CF_TimeAvg(:,:))

!$acc enter data copyin(wdist,wsin,wcos,fdamp(:,istart:iend),ypluss(:,istart:iend),upluss(:,istart:iend))	if(wall_model=='ON')

!$acc enter data copyin(yplus_print(:,:,istart:iend),uplus_print(:,:,istart:iend),dist_print(:,:,istart:iend)) if(filer_near_wall=='ON')
...
!$acc enter data copyin(theta(:,:,istart-2:iend+2),theta0(:,:,istart-2:iend+2), &
!$acc	theta1(:,:,istart-2:iend+2),dtheta_dt(:,:,istart-2:iend+2),thetastar(:,:,istart-2:iend+2), &
!$acc	thetastar1(:,:,istart-2:iend+2),Ftheta(:,:,istart-2:iend+2))										if(heat_transfer=='CONJ' .OR. heat_transfer=='ISOT')
...
do while (time <= total_time)                                                                     
	...
	inv_dt = 1.d0/dt					!dt is determined in calcul_new_velocity.f90 
   ...
   !time1end = MPI_WTIME()
	  ...
	  !$acc update self(u(:,:,istart:istart+1),v(:,:,istart:istart+1),w(:,:,istart:istart+1), &
	  !$acc				u(:,:,iend-1:iend),    v(:,:,iend-1:iend),    w(:,:,iend-1:iend)   )  if(nproc>1)
      icount = 2*(nx+4)*(ny+4)
      itag = 100
      ...
      call MPI_BARRIER(MPI_COMM_WORLD, ierr)
	  !$acc update device(u(:,:,iend+1:iend+2),    v(:,:,iend+1:iend+2),    w(:,:,iend+1:iend+2))     if(nproc>1 .AND. myid/=(nproc-1))
	  !$acc	update device(u(:,:,istart-2:istart-1),v(:,:,istart-2:istart-1),w(:,:,istart-2:istart-1)) if(nproc>1 .AND. myid/=0)
      !>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<

   !time2start = MPI_WTIME()
      !------------------------------------------------------------------------------------------------------!

!call nvtxStartRange("SGS model")
	  if (LES_mode /= 'OFF') then						! *** SGS_model.f90 ***
		if (LES_mode=='WALE') then
			call CalculateWALEViscosity()  				! calculate WALE nut	
		elseif (LES_mode=='SMAG') then
			call CalculateSmagorinskyViscosity()		! calculate Smagorinsky nut
		endif
!call nvtxEndRange		 

!call nvtxStartRange("Wall model")
		if (wall_model=='ON') then	! .AND. time > 50.d0) then ! .AND. istep == INT((10.d0*dt)/dt)) then
...
!call nvtxEndRange	
      ...
      call MPI_BARRIER(MPI_COMM_WORLD, ierr)
	  !$acc update device(nut(:,:,iend+1)) if(nproc>1 .AND. myid/=(nproc-1))     
      !>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
!call nvtxStartRange("AB")
      ...
      !------------------------------------------------------------------------------------------------------!
      call AdamsBashforth()                   ! calculate time evolution				*** AdamsBashforth.f90 ***
      !------------------------------------------------------------------------------------------------------!
!call nvtxEndRange
   !time2end = MPI_WTIME()
...
!call nvtxEndRange
      ...
      call MPI_BARRIER(MPI_COMM_WORLD, ierr)
	  !$acc update device(w0(:,:,istart-1)) if(nproc>1 .AND. myid/=0)     
      !>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
!call nvtxEndRange
      ...
      call MPI_BARRIER(MPI_COMM_WORLD, ierr)
	  !$acc update device(p(:,:,iend+1)) if(nproc>1 .AND. myid/=(nproc-1))
      !>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
!call nvtxEndRange
   ...
   !time4end = MPI_WTIME() 
	  ...
	  !>>>>>>>>>>>>>>>>> data transfer among MPI processes <<<<<<<<<<<<<<<<<<<
	  !$acc update self(FZ(:,:,iend)) if(nproc>1)
	  icount = (nx+4)*(ny+4)
	  itag = 109
	  call MPI_SENDRECV( FZ(-1,-1,iend),     icount, MPI_REAL8, r_nbr, itag, &
					     FZ(-1,-1,istart-1), icount, MPI_REAL8, l_nbr, itag, MPI_COMM_WORLD, status, ierr )
      
	  call MPI_BARRIER(MPI_COMM_WORLD, ierr)
	  !$acc update device(FZ(:,:,istart-1)) if(nproc>1 .AND. myid/=0)     
	  !>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
!call nvtxStartRange("virtual force integrator")
	  ...
	  !		call virtualForce_singleVIV()
	  !		call virtualForce_tandemVIV()
	  !	    call virtualForceTorqueIntegrator() ! CD, CL, CT, and CP
	  !	    call virtualForce_Magnus1blade() ! CD, CL, CT, and CP
	  ...
	  !		call virtualForceIntegrator_shear
      !------------------------------------------------------------------------------------------------------!
!call nvtxEndRange
...
!call nvtxEndRange     
   !time5end = MPI_WTIME()     
      ...
      !------------------------------------------------------------------------------------------------------!
	  ! HEAT TRANSFER
	    if(heat_transfer=='CONJ' .OR. heat_transfer=='ISOT') Then
			...
			!>>>>>>>>>>>>>>>>> data transfer among MPI processes <<<<<<<<<<<<<<<<<<<
			!$acc update self(theta(:,:,istart:istart+1),theta(:,:,iend-1:iend))  if(nproc>1)
			icount = 2*(nx+4)*(ny+4)
			itag = 140
			call MPI_SENDRECV( theta(-1,-1,istart), icount, MPI_REAL8, l_nbr, itag, &
							   theta(-1,-1,iend+1), icount, MPI_REAL8, r_nbr, itag, MPI_COMM_WORLD, status, ierr )
			itag = 141
			call MPI_SENDRECV( theta(-1,-1,iend-1),   icount, MPI_REAL8, r_nbr, itag, &
							   theta(-1,-1,istart-2), icount, MPI_REAL8, l_nbr, itag, MPI_COMM_WORLD, status, ierr )

			call MPI_BARRIER(MPI_COMM_WORLD, ierr)
			!$acc update device(theta(:,:,iend+1:iend+2)    )     if(nproc>1 .AND. myid/=(nproc-1))
			!$acc update device(theta(:,:,istart-2:istart-1)) if(nproc>1 .AND. myid/=0)
			!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<    
	  
			!------------------------------------------------------------------------------------------------!	  
			call discretisation_QUICK_HT() 	! calculate theta field 						*** heat_transfer.f90 ***
			!------------------------------------------------------------------------------------------------!	 

			!------------------------------------------------------------------------------------------------!	 
			call AdamsBashforth_HT() 		! calculate time evolution						*** heat_transfer.f90 ***
			!------------------------------------------------------------------------------------------------!	 

			!------------------------------------------------------------------------------------------------!	 
			if(heat_transfer=='ISOT') call calcul_new_theta()								!	*** heat_transfer.f90 ***
			!------------------------------------------------------------------------------------------------!	 

			!------------------------------------------------------------------------------------------------!	 		
			call thermalForceIntegrator()												!	*** heat_transfer.f90 ***
			!------------------------------------------------------------------------------------------------!	 		

			!------------------------------------------------------------------------------------------------!	 		
			call Updating_theta()														!	*** heat_transfer.f90 ***
			!------------------------------------------------------------------------------------------------!	 

			call theta_boundary_conditions() 											!	*** boundary_conditions.f90 ***
		endif
      !------------------------------------------------------------------------------------------------------!
...
!call nvtxStartRange("output files")
	...
	!$acc update self(u(:,:,istart:iend),v(:,:,istart:iend),w(:,:,istart:iend),p(:,:,istart:iend),ETA)        
      ...
      !>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
		...
		if (heat_transfer/='OFF') then
			!$acc update self(theta(:,:,istart:iend),dtheta_dt(:,:,istart:iend))
			!>>>>>>>>>>>>>>>> send results back to Master process <<<<<<<<<<<<<<<<<<
			if(myid>master)then
				icount = igcount*(nx+4)*(ny+4)
				itag = 142
				call MPI_SEND( theta(-1,-1,istart), icount, MPI_REAL8, master, itag, MPI_COMM_WORLD, ierr )
				itag = 143
				call MPI_SEND( dtheta_dt(-1,-1,istart), icount, MPI_REAL8, master, itag, MPI_COMM_WORLD, ierr )
			end if
			...
			if(myid==master)then
			  do i = 1, (nproc-1)
				icount = gcount(i)*(nx+4)*(ny+4)
				itag = 142
				call MPI_RECV( theta(-1,-1,gstart(i)), icount, MPI_REAL8, i, itag, MPI_COMM_WORLD, status, ierr )
				itag = 143
				call MPI_RECV( dtheta_dt(-1,-1,gstart(i)), icount, MPI_REAL8, i, itag, MPI_COMM_WORLD, status, ierr )     
			  end do
			end if
			call MPI_BARRIER(MPI_COMM_WORLD, ierr) 
			!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<		
...
!call nvtxStartRange("write_result")         
        if (time > startfiler3d_time-dt .AND. time >= next_filer3d-1.0d-10) then
			...
			if(filer3d=='ON') then
				...
				if(heat_transfer/='OFF') call filereachtime3d_Temp()
				next_filer3d = next_filer3d + filer3d_int
			endif
        end if 

        if (time > startfiler2d_time-dt .AND. time >= next_filer2d-1.0d-10) then
			if(filer2d=='MID' .OR. filer2d=='ON') then
				call filereachtime2d_Mid()				! write filer 2D							*** filer.f90 ***
				if(heat_transfer/='OFF') call filer2dTemp_Mid()
				next_filer2d = next_filer2d + filer2d_int
			elseif (filer2d=='AVG') then
				call filereachtime2d_Avg()				! write filer 2D							*** filer.f90 ***
				if(heat_transfer/='OFF') call filer2dTemp_Avg()
				next_filer2d = next_filer2d + filer2d_int
			endif
        end if 
!call nvtxEndRange 
...
!call nvtxStartRange("write_backup")		 
        if (time >= next_backup-1.0d-10) then
			...
			if(heat_transfer/='OFF') call backupfile_HT() 
			lastbackuptime = time
...
! ------------------------------- Wall model output ----------------------------------
	...
	if (filer_near_wall=='ON' .AND. time > startfilerNW_time-dt .AND. time >= next_filerNW-1.0d-10) then  
	!$acc update self(yplus_print(:,:,istart:iend),uplus_print(:,:,istart:iend),dist_print(:,:,istart:iend),nut(:,:,istart:iend),ETA)
	  ...
	  call acc_set_device_type(acc_device_host)
			...
			next_filerNW = next_filerNW + filerNW_int
	  
	  call acc_set_device_num(myid,acc_device_nvidia)
      end if
	
	call MPI_BCAST(next_filerNW, 1, MPI_REAL8, 0, MPI_COMM_WORLD, ierr)
	end if
...
!$acc exit data if(heat_transfer=='CONJ' .OR. heat_transfer=='ISOT')
!$acc exit data if(filer_running_avg=='FULL')
!$acc exit data if(filer_running_avg=='STANDARD' .OR. filer_running_avg=='ON')
!$acc exit data if(filer_near_wall=='ON')
!$acc exit data if(wall_model=='ON')
!$acc end data
