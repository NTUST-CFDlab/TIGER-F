...
!$OMP PARALLEL DO PRIVATE(Yf,Zf) REDUCTION(max : az_max,ay_max) REDUCTION(min : az_min,ay_min) COLLAPSE(2)
...
!$OMP PARALLEL DO PRIVATE(m,n,SY,SZ,Yf,Zf) COLLAPSE(2)
!$acc parallel vector_length(32)
...
!$OMP END PARALLEL DO  
	...
	!-------------- Calculate moment of inertia -------------------
		...
		if( istep == 0 )then
			...
			!$OMP PARALLEL DO   REDUCTION(+: I_moment) COLLAPSE(3)
			!$acc parallel loop reduction(+: I_moment) collapse(3) gang vector
			do k = kBgnVOS-2,kEndVOS+2
				do j = jBgnVOS-2,jEndVOS+2
					do i=1,nx
						I_moment = I_moment+den_sol*ETA(i,j,k)*((ys(j) - y0_t)**2 +(zs(k)-z0_t)**2)*iDx(i)*iDy(j)*iDz(k)
					end do
				end do
			end do
			!$acc end parallel
		...
		!write(*,*) 'Moment of inertia = ', I_moment
...
!$OMP PARALLEL DO PRIVATE(Yf1,Zf1,Yf2,Zf2) REDUCTION(max : az_max,ay_max) REDUCTION(min : az_min,ay_min) COLLAPSE(2)
!$acc parallel vector_length(32)
!$acc loop        private(Yf1,Zf1,Yf2,Zf2) reduction(max : az_max,ay_max) reduction(min : az_min,ay_min) collapse(2) gang vector
...
do k=1,nz
	...
	if( (Zf1**2 + Yf1**2) .LE. r**2 .OR. (Zf2**2 + Yf2**2) .LE. r**2) then
		ETA(i,j,k)=1.D0
		az_min = MIN(k,az_min)
		az_max = MAX(k,az_max)
		ay_min = MIN(j,ay_min)
...
!$OMP PARALLEL DO PRIVATE(m,n,SY,SZ,Yf1,Zf1,Yf2,Zf2) COLLAPSE(2)
!$acc parallel vector_length(32)
!$acc loop        private(m,n,SY,SZ,Yf1,Zf1,Yf2,Zf2) collapse(2) gang
do k=kBgnVOS,kEndVOS
do j=jBgnVOS,jEndVOS
	...
	if(ETA_2D(k,j) == 1) then
		    ...
		    ETA(i,j,k) = 0.d0
            !$acc loop private(n,Yf1,Zf1,Yf2,Zf2) collapse(2) vector
            do m=1,nSubGrids_f
            do n=1,nSubGrids_f
...
!$OMP PARALLEL DO PRIVATE(Yf,Zf,z_aoa1,y_aoa1,z_trans1,y_trans1,z_aoa2,y_aoa2,z_trans2,y_trans2,z_aoa3,y_aoa3,z_trans3,y_trans3) &
...
!$OMP PARALLEL DO PRIVATE(m,n,Yf,Zf,SY,SZ,z_aoa1,y_aoa1,z_trans1,y_trans1,z_aoa2,y_aoa2,z_trans2,y_trans2,z_aoa3,y_aoa3,z_trans3,y_trans3) COLLAPSE(2)
!$acc parallel vector_length(32)
	...
	!-------------- Calculate moment of inertia -------------------
		...
		if( istep == 0 )then
			...
			do k = kBgnVOS-2,kEndVOS+2
				do j = jBgnVOS-2,jEndVOS+2
					do i=1,nx
						I_moment = I_moment+den_sol*ETA(i,j,k)*((ys(j) - y0)**2 +(zs(k)-z0)**2)*iDx(i)*iDy(j)*iDz(k)
					end do
				end do
			end do
			!$acc end parallel
...
!$OMP PARALLEL DO PRIVATE(Yf,Zf,z_aoa1,y_aoa1,z_trans1,y_trans1,z_aoa2,y_aoa2,z_trans2,y_trans2) &
!$OMP			  REDUCTION(max : az_max,ay_max) REDUCTION(min : az_min,ay_min) COLLAPSE(2)
!$acc parallel vector_length(32)
...
!$OMP PARALLEL DO PRIVATE(m,n,Yf,Zf,SY,SZ,z_aoa1,y_aoa1,z_trans1,y_trans1,z_aoa2,y_aoa2,z_trans2,y_trans2) COLLAPSE(2)
!$acc parallel vector_length(32)
...
!$OMP PARALLEL DO PRIVATE(Yf,Zf,z_aoa1,y_aoa1,z_trans1,y_trans1) &
!$OMP			  REDUCTION(max : az_max,ay_max) REDUCTION(min : az_min,ay_min) COLLAPSE(2)
!$acc parallel vector_length(32)
...
!$OMP PARALLEL DO PRIVATE(m,n,Yf,Zf,SY,SZ,z_aoa1,y_aoa1,z_trans1,y_trans1) COLLAPSE(2)
!$acc parallel vector_length(32)
...
!$OMP PARALLEL DO PRIVATE(Yf,Zf,z_aoa1,y_aoa1) &
!$OMP			  REDUCTION(max : az_max,ay_max) REDUCTION(min : az_min,ay_min) COLLAPSE(2)
!$acc parallel vector_length(32)
...
!$OMP PARALLEL DO PRIVATE(m,n,Yf,Zf,SY,SZ,z_aoa1,y_aoa1) COLLAPSE(2)
!$acc parallel vector_length(32)
...
!$OMP PARALLEL DO PRIVATE(Yf,Zf,z_trans1) &
!$OMP			  REDUCTION(max : az_max,ay_max) REDUCTION(min : az_min,ay_min) COLLAPSE(2)
!$acc parallel vector_length(32)
...
!$OMP PARALLEL DO PRIVATE(m,n,Yf,Zf,SY,SZ,z_trans1,y_trans1) COLLAPSE(2)
!$acc parallel vector_length(32)
...
! !$OMP PARALLEL DO PRIVATE(Yf,Zf,z_aoa1,y_aoa1,z_trans_,y_trans_,z_trans1,y_trans1,z_aoa2,y_aoa2,z_trans2,y_trans2) &
! !$OMP			  REDUCTION(max : az_max,ay_max) REDUCTION(min : az_min,ay_min) COLLAPSE(2)
! !$acc parallel vector_length(32)
...
! !$OMP PARALLEL DO PRIVATE(m,n,Yf,Zf,SY,SZ,z_aoa1,y_aoa1,z_trans_,y_trans_,z_trans1,y_trans1,z_aoa2,y_aoa2,z_trans2,y_trans2) COLLAPSE(2)
! !$acc parallel vector_length(32)
...
! !$OMP PARALLEL DO PRIVATE(Yf,Zf,z_aoa1,y_aoa1,z_trans_,y_trans_,z_trans1,y_trans1) &
! !$OMP			  REDUCTION(max : az_max,ay_max) REDUCTION(min : az_min,ay_min) COLLAPSE(2)
! !$acc parallel vector_length(32)
...
! !$OMP PARALLEL DO PRIVATE(m,n,Yf,Zf,SY,SZ,z_aoa1,y_aoa1,z_trans_,y_trans_,z_trans1,y_trans1) COLLAPSE(2)
! !$acc parallel vector_length(32)
...
!$OMP PARALLEL DO REDUCTION(max : az_max,ay_max) REDUCTION(min : az_min,ay_min) COLLAPSE(2)
!$acc parallel vector_length(32)
...
!$OMP PARALLEL DO PRIVATE(m,n,SY,SZ) COLLAPSE(2)
!$acc parallel vector_length(32)
...
! !$OMP PARALLEL DO               PRIVATE(DIAGONAL,DISTANCE,SDIST,l,m,n,dxg,dyg,dzg,SX,SY,SZ,xi) COLLAPSE(3)
...
!$OMP PARALLEL DO      PRIVATE(Xf,Yf,Zf) REDUCTION(max : az_max,ay_max,ax_max) REDUCTION(min : az_min,ay_min,ax_min) COLLAPSE(3)
!$acc parallel vector_length(32)
...
!$OMP PARALLEL DO      PRIVATE(l,m,n,SX,SY,SZ,Xf,Yf,Zf,xi) COLLAPSE(3)
!$acc parallel vector_length(32)
...
! ! Setting the boundary for subgrids
    ...
    ! do i=0,(nproc-1)
        ...
        ! if(i < Zr_vos) then
            ...
            ! gend_vos(i) = gstart_vos(i) + Zdv_vos
        ! else
            ! gstart_vos(i) = kBgnVOS + i * Zdv_vos + Zr_vos
            ! gend_vos(i) = gstart_vos(i) + Zdv_vos - 1
        ! end if
        
        ! gcount_vos(i) = gend_vos(i) - gstart_vos(i) + 1

    ! end do

    ! !----------for nz vos----------!
    ! istart_vos = gstart_vos(myid)  !
    ! iend_vos = gend_vos(myid)     !
...
!$OMP PARALLEL DO PRIVATE(nn,sphere) REDUCTION(max : az_max,ay_max,ax_max) REDUCTION(min : az_min,ay_min,ax_min) COLLAPSE(3)
!$acc parallel vector_length(32)
...
!$OMP PARALLEL DO PRIVATE(l,m,n,nn,SX,SY,SZ,xi,sphere) COLLAPSE(3)
!$acc parallel vector_length(32)
