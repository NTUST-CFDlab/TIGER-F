...
implicit none
integer           :: n
integer(4)        :: num_facets
integer           :: actual_size, expected_size
character(len=80) :: useless
real(4)           :: bin_n(3), bin_v1(3), bin_v2(3), bin_v3(3)
integer(2)        :: attr 
real*4            :: tun       
...
inquire(file=TRIM(stl_file), size=actual_size)
open(unit=9, file=TRIM(stl_file), status='old', access='stream', form='unformatted')
read(9) useless
read(9) num_facets
close(9)

expected_size = 84 + 50 * num_facets

if (actual_size == expected_size) then
    if(myid==master)then
		open (62,file='geometry_info.dat',position='append')
		write(62,*)'========================================='
		write(62,*)' [Auto-Detect] Format: Binary STL'
		write(62,*)'========================================='
		write(62,*)'                   '
    endif
    ...
    allocate(iFN(3,nf))                     ! Size of arrays determined for facet normal & vertices
    allocate(iPx(3*nf))
    allocate(iPy(3*nf))
    allocate(iPz(3*nf))
    ...
    do n = 1, nf
        ...
        iFN(1,n) = real(bin_n(1), 8)		! facet normal nx ny nz (or swapped axis)
        iFN(2,n) = real(bin_n(2), 8)
        iFN(3,n) = real(bin_n(3), 8)
        
        iPx((n-1)*3+1) = real(bin_v1(1), 8)	! vertex v1x v1y v1z (or swapped axis)
        iPx((n-1)*3+2) = real(bin_v2(1), 8)	
        iPx((n-1)*3+3) = real(bin_v3(1), 8)			
		
        iPy((n-1)*3+1) = real(bin_v1(2), 8)	! vertex v2x v2y v2z (or swapped axis)
        iPy((n-1)*3+2) = real(bin_v2(2), 8)
        iPy((n-1)*3+3) = real(bin_v3(2), 8)
		
        iPz((n-1)*3+1) = real(bin_v1(3), 8)	! vertex v3x v3y v3z (or swapped axis)
        iPz((n-1)*3+2) = real(bin_v2(3), 8)
        iPz((n-1)*3+3) = real(bin_v3(3), 8)
    end do
    ...
    if(myid==master)then
		open (62,file='geometry_info.dat',position='append')
		write(62,*)'========================================='
		write(62,*)' [Auto-Detect] Format: ASCII STL'
		write(62,*)'========================================='
		write(62,*)'                   '
    endif
    ...
    !    iPz((n-1)*3+1)=-iPz((n-1)*3+1)
    !    iPz((n-1)*3+2)=-iPz((n-1)*3+2)
    !    iPz((n-1)*3+3)=-iPz((n-1)*3+3)
    !enddo
	!!$acc end parallel
    !!$OMP END PARALLEL DO
...
!................ Bounding BOX  ......................
	...
	allocate(nk(jbgnvos:jendvos,istart_vos:iend_vos) )
	allocate(n_(jbgnvos:jendvos,istart_vos:iend_vos,1:nf) )


!$acc data create(nk(jbgnvos:jendvos,istart_vos:iend_vos),n_(jbgnvos:jendvos,istart_vos:iend_vos,1:nf))


    !$OMP PARALLEL DO      PRIVATE(nn,n) COLLAPSE(2)
...
! !------------------------------------  VOS using RayCasting & Subgrid -----------------------------------------
    ...
    ! !$OMP PARALLEL DO      PRIVATE(nsy, nsz, n, nc, bubi, bubj, bubble, i, ETA_sub, ray_inter, Zsub, Ysub, c1, c2, c3, ETAs) COLLAPSE(2)
	! !$acc parallel vector_length(64)
...
ETA_sub = 0.
    ...
    !$OMP PARALLEL DO      PRIVATE(nsy, nsz, n, nn, nc, bubi, bubj, bubble, i, ray_inter, Zsub, Ysub, c1, c2, c3, ETAs) COLLAPSE(2)
