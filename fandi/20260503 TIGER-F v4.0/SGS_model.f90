...
!$acc	u(:,:,istart-2:iend+2),v(:,:,istart-2:iend+2),w(:,:,istart-2:iend+2))
  ...
  if (ETA(i,j,k) >= 0.5d0) then		! Ommitting solid & interface cells
  
	nut(i,j,k) = 0.d0
	cycle
	
  else if(wall_model=='OFF' .AND. ANY(ETA(i-1:i+1, j-1:j+1, k-1:k+1) > 0.d0)) then		! Suppressing spurious velocity gradients due to step-wise interface.
  ...
  else if(wall_model=='ON' .AND. ANY(ETA(i-1:i+1, j-1:j+1, k-1:k+1) > 0.d0)) then
	...
	!Shear components (central difference)
	du_dy = 0.5d0*(u(i-1,j+1,k) + u(i,j+1,k) - u(i-1,j-1,k) - u(i,j-1,k)) * inv_dy
	dv_dx = 0.5d0*(v(i+1,j-1,k) + v(i+1,j,k) - v(i-1,j-1,k) - v(i-1,j,k)) * inv_dx

	du_dz = 0.5d0*(u(i-1,j,k+1) + u(i,j,k+1) - u(i-1,j,k-1) - u(i,j,k-1)) * inv_dz
	dw_dx = 0.5d0*(w(i+1,j,k-1) + w(i+1,j,k) - w(i-1,j,k-1) - w(i-1,j,k)) * inv_dx

	dv_dz = 0.5d0*(v(i,j-1,k+1) + v(i,j,k+1) - v(i,j-1,k-1) - v(i,j,k-1)) * inv_dz
	dw_dy = 0.5d0*(w(i,j+1,k-1) + w(i,j+1,k) - w(i,j-1,k-1) - w(i,j-1,k)) * inv_dy
...
! Applying damping function to solid boundary cells	
		...
		if ( wall_model=='OFF' .AND. 0.d0 < ETA(i,j,k) .AND. ETA(i,j,k) < 1.d0) then
