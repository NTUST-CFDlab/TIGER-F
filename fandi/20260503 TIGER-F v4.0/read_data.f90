...
open (28,file='DYNAMIC.bak',form='unformatted',status='old',access='stream',iostat=ierr)
if (ierr /= 0) then
	open (28,file='STATIC.bak',form='unformatted',status='old',access='stream',iostat=ierr)
    if (ierr /= 0) then
	  if(myid==master)then
...
subroutine read_backupfile_HT()
use variables
implicit none

open (32,file='THERMAL.bak',form='unformatted',status='old',access='stream',iostat=ierr)
if (ierr /= 0) then
	if(myid==master)then
      write(*,*) 'Error: could not open file'
      stop
	...
	inputfile = 'THERMAL.bak'
endif

read(32) inblocks
read(32) inx, iny, inz
read(32) temp, temp, temp, temp
read(32) ( ( ( ( Qout(i,j,k,h), i = 1, nx), j = 1, ny), k = 1, nz), h = 1, 5 )
close(32)


   do k=1,nz; do j=1,ny; do i=1,nx  

      theta(i,j,k) = Qout(i,j,k,1)
      dtheta_dt(i,j,k) = Qout(i,j,k,2)
...
end subroutine read_backupfile_HT
...
open (29,file='RUNNING_AVG.bak',form='unformatted',status='old',access='stream',iostat=ierr)
if (ierr /= 0) then
	if(myid==master)then
      write(*,*) 'Error: could not open file'
      stop
...
open (30,file='RUNNING_AVG1.bak',form='unformatted',status='old',access='stream',iostat=ierr)
if (ierr /= 0) then
	if(myid==master)then
      write(*,*) 'Error: could not open file'
      stop
...
open (31,file='RUNNING_AVG2.bak',form='unformatted',status='old',access='stream',iostat=ierr)
if (ierr /= 0) then
	if(myid==master)then
      write(*,*) 'Error: could not open file'
      stop
