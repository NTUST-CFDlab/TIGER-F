...
integer		:: jj,kk,col,line
real*8		:: zb_nut,yb_nut,zr_nut,yr_nut,nut_sgs
real*8, dimension(1:2,1:2) 	:: f_wr,f_vr
real*8		:: wr,vr
real*8		:: uc_r,theta_b
real*8		:: dist_r,dist_b
real*8		:: yplusB,uPlusB,weight,step
...
!$OMP PARALLEL DO PRIVATE(zb_nut,yb_nut,zr_nut,yr_nut,nut_sgs,theta_b,kk,jj,f_wr,f_vr,wr,vr,uc_r, &
!$OMP					dist_r,dist_b,kUu,fkUu,NR_f,NR_df,uTau_,NRit,NRerr,uTau,step,yplusB,uPlusB,weight) &
!$OMP					reduction(+:yplusSUM,ypluspts) collapse(3)
!$acc parallel
!$acc loop independent private(zb_nut,yb_nut,zr_nut,yr_nut,nut_sgs,theta_b,kk,jj,f_wr,f_wr,f_vr,vr,uc_r, &
!$acc					dist_r,dist_b,kUu,fkUu,NR_f,NR_df,uTau_,NRit,NRerr,uTau,step,yplusB,uPlusB,weight) &
			...
			if((zr_nut .GE. Zs(kk)) .AND. (zr_nut .LT. Zs(kk+1))) then
				...
				do jj=jBgnVOS-INT(c_ref+1.),jEndVOS+INT(c_ref+1.)
					if((yr_nut .GE. Ys(jj)) .AND. (yr_nut .LT. Ys(jj+1))) then
					  do line=1,2 ; do col=1,2
						f_wr(col,line) = 0.5d0*(w(i,jj+(col-1),kk+(line-1)) + w(i,jj  +(col-1),kk-1+(line-1)))
						f_vr(col,line) = 0.5d0*(v(i,jj+(col-1),kk+(line-1)) + v(i,jj-1+(col-1),kk  +(line-1)))
					  enddo ; enddo
						
						wr = bilinear_interp(yr_nut,zr_nut,Ys(jj),Ys(jj+1),Zs(kk),Zs(kk+1),f_wr)
						vr = bilinear_interp(yr_nut,zr_nut,Ys(jj),Ys(jj+1),Zs(kk),Zs(kk+1),f_vr)
						...
						if(yplusB .LE. yplusLam) then
							uPlusB = yplusB
							nut(i,j,k) = 0.d0
						elseif(yplusB .LE. 300.d0) then
							! Calculate uPlus at point B (Using Musker's Explicit Expression for the Smooth Wall
							! Velocity Distribution in a Turbulent Boundary Layer)
							uPlusB = 5.424d0 * ATAN((2.0d0 * yplusB - 8.15d0) / 16.7d0) &
								+ log10(((yplusB + 10.6d0)**9.6d0) / ((yplusB**2 - 8.15d0 * yplusB + 86.0d0)**2))- 3.52d0

							! Calculate nut ( nut = nu * (dy+/du+ - 1)
							nut(i,j,k) = DMAX1(0.d0 , uTau*dist_b/(uPlusB + ROOTVSMALL) - nu)
						else
							nut(i,j,k) = nut_sgs
						endif
...
! write(*,'(3(3x,F15.10))') yplusB,uPlusB,uTau
						
! if (myid==master .AND. i==1 .AND. (istep == 100 .OR. mod(istep,1000)==0)) then
! write(84,'(I6,6(3x,F15.10))') NRit,yplusB,uPlusB,uTau,nut(i,j,k)
! endif
					endif
				enddo
			endif
