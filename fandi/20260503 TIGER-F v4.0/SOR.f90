...
do while (pChangeMax_>zeta .AND. ik < itmax)
	...
	!$acc update self(p(:,:,istart),p(:,:,iend)) if(nproc>1)
    icount = (nx+4)*(ny+4)
	...
	!$acc update device(p(:,:,istart-1)) if(nproc>1 .AND. myid/=0)
    !>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
! Red-Black ordering (RED)
    ...
    !$OMP PARALLEL
	!$OMP DO               PRIVATE(pNEW) REDUCTION(max:pChangeMax) reduction(+:pSum) COLLAPSE(3)
	!$acc parallel vector_length(512)
	!$acc loop independent private(pNEW) reduction(max:pChangeMax) reduction(+:pSum) collapse(3) gang vector
    do k=istart,iend  
		do j=1,ny
            do i=1,nx
...
! Red-Black ordering (BLACK)
    ...
    if(myid==master)then
        ...
        call MPI_ALLREDUCE( pSum, pSum_, 1, MPI_REAL8, MPI_SUM, MPI_COMM_WORLD, ierr )
...
! do while (pChangeMax_>zeta .AND. ik < itmax)
    ...
    ! !>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        ...
        ! !$OMP PARALLEL DO PRIVATE(pNEW) COLLAPSE(3)
        ! do k=istart,iend 
        ! do j=1,ny
