...
!$acc data present(iDx,iDy,iDz,Dxs,Dys,Dzs,ETA,Viseff(:,:,istart:iend), &
...
!$acc create(dudx(:,:,istart:iend,:),dvdx(:,:,istart:iend,:),dwdx(:,:,istart:iend,:))
   ...
   !$OMP PARALLEL
    ...
    !$acc parallel loop independent private(mutsgs,delta) collapse(3) gang vector
    do k=istart,iend
    do j=1,ny; do i=1,nx

    mutsgs = dabs(&
                 2.d0*dudx(i,j,k,1)**2 + 2.d0*dvdx(i,j,k,2)**2 + 2.d0*dwdx(i,j,k,3)**2 + &
...
! Applying damping function to solid boundary cells	
	  ...
	  if ((0.5d0*(ETA(i,j,k)+ETA(i,j+1,k))) > 0.d0 .AND. (0.5d0*(ETA(i,j,k)+ETA(i,j+1,k))) < 1.d0) then

      mutsgs = (Cs*delta*fwall)**2*dsqrt(mutsgs) 

      else

      mutsgs = (Cs*delta)**2*dsqrt(mutsgs)    
