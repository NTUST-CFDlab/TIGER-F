...
subroutine RB_SOR_HYB() 
    ...
    !----------data transfer to master----------!
      ...
      if(myid>master)then
         ...
         itag = 260
         call MPI_SEND( mChange(1,1,istart), icount, MPI_REAL8, master, itag, MPI_COMM_WORLD, ierr )
      end if
      ...
      if(myid==master)then
         do i = 1, (nproc-1)
			...
			itag = 260
			call MPI_RECV( mChange(1,1,gstart(i)), icount, MPI_REAL8, i, itag, MPI_COMM_WORLD, status, ierr )			
         end do
      end if
      call MPI_BARRIER(MPI_COMM_WORLD, ierr)
