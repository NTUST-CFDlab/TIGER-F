	...
	if (istep == 100) then 
		...
		do k=kBgnVOS-3,kEndVOS+3
			do j=jBgnVOS-3,jEndVOS+3
				...
				!write(82,'(3(3x,F15.10))') Zs(k),Ys(j),REAL(fric_coeff(nx/2,j,k))
				write(82,'(3G18.10E3)') ACOS(-(Zs(k)-z0)/rad)*180.d0/PI,fric_coeff(nx/2,j,k),moment_dist(nx/2,j,k)
				endif
			enddo
		enddo
		...
		do k=kBgnVOS-3,kEndVOS+3
			do j=jBgnVOS-3,jEndVOS+3
				...
				!write(82,'(3(3x,F15.10))') Zs(k),Ys(j),REAL(fric_coeff(nx/2,j,k))
				write(82,'(3G18.10E3)') ACOS((Zs(k)-z0)/rad)*180.d0/PI+180.d0,fric_coeff(nx/2,j,k),moment_dist(nx/2,j,k)
				endif
			enddo
		enddo
