...
!South horizontal wall
   ...
   !$acc loop independent collapse(2) gang vector 
	...
	do i=-1,nx+2
  	p(i,0,k)=p(i,1,k)
  	p(i,-1,k)=p(i,0,k)
  	! p(i,0,k)=p(i,ny,k) ! Periodic boundary condition
  	! p(i,-1,k)=p(i,ny-1,k) ! Periodic boundary condition
	end do
  	end do
   !$OMP END DO
...
!North horizontal wall
   ...
   !$acc loop independent collapse(2) gang vector 
	...
	do i=-1,nx+2
  	...
  	! p(i,ny+1,k)=p(i,1,k) ! Periodic boundary condition
  	! p(i,ny+2,k)=p(i,2,k) ! Periodic boundary condition
	end do
  	end do
  !$acc end parallel
  !$OMP END DO
...
!Back horizontal wall
	...
	if(myid==0) then
		...
		!$acc parallel loop independent collapse(2) gang vector
			...
			p(i,j,0)=p(i,j,1)
			p(i,j,-1)=p(i,j,0)
		    ! p(i,j,0)=p(i,j,nz) ! Periodic boundary condition
			! p(i,j,-1)=p(i,j,nz-1) ! Periodic boundary condition
			end do
			end do
		!$acc end parallel   
		!$OMP END PARALLEL DO
...
!Front horizontal wall
	if(myid==nproc-1) then
		...
		!$acc parallel loop independent collapse(2) gang vector
		    ...
		    ! p(i,j,nz+1)=p(i,j,1) ! Periodic boundary condition
			! p(i,j,nz+2)=p(i,j,2) ! Periodic boundary condition
			end do
			end do
		!$acc end parallel
		!$OMP END PARALLEL DO
