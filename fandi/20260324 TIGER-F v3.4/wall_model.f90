...
function bilinear_interp(y, z, y0, y1, z0, z1, f00, f10, f01, f11) result(val)
  implicit none
!$acc routine
...
integer		:: jj,kk
real*8		:: zb_nut,yb_nut,zr_nut,yr_nut,nut_sgs
real*8		:: wr00,wr10,wr01,wr11,vr00,vr10,vr01,vr11
real*8		:: wr,vr,bilinear_interp
real*8		:: uc_r,uc_b,theta_b
real*8		:: dist_r,dist_b
real*8		:: yplusR,yplusR_,yplusB,uPlusB,weight,step
...
!$acc data present(ETA,Zs,Ys,v(:,:,istart-2:iend+2),w(:,:,istart-2:iend+2),nut(:,:,istart:iend))
!$acc data present(yplus_print(:,:,istart:iend),uplus_print(:,:,istart:iend),dist_print(:,:,istart:iend)) if(filer_near_wall=='ON')
...
!$OMP PARALLEL DO PRIVATE(zb_nut,yb_nut,zr_nut,yr_nut,nut_sgs,theta_b,kk,jj,wr00,wr10,wr01,wr11,vr00,vr10,vr01,vr11,wr,vr,uc_r, &
!$OMP					dist_r,dist_b,kUu,fkUu,NR_f,NR_df,uTau_,yplusR,yplusR_,NRit,NRerr,uTau,step,yplusB,uPlusB,uc_b,weight) &
!$OMP					reduction(+:yplusSUM,ypluspts) collapse(3)
!$acc parallel
!$acc loop independent private(zb_nut,yb_nut,zr_nut,yr_nut,nut_sgs,theta_b,kk,jj,wr00,wr10,wr01,wr11,vr00,vr10,vr01,vr11,wr,vr,uc_r, &
!$acc					dist_r,dist_b,kUu,fkUu,NR_f,NR_df,uTau_,yplusR,yplusR_,NRit,NRerr,uTau,step,yplusB,uPlusB,uc_b,weight) &
!$acc					reduction(+:yplusSUM,ypluspts) collapse(3) gang vector
...
do k=istart,iend
	...
	if(ETA(i,j,k) .LT. 0.5d0 .AND. ((zb_nut-z0_t)**2 + (yb_nut-y0_t)**2) .LT. (r+c_ref*dySml)**2) then
			...
			if((zr_nut .GE. Zs(kk)) .AND. (zr_nut .LT. Zs(kk+1))) then
				...
				do jj=jBgnVOS-INT(c_ref+1.),jEndVOS+INT(c_ref+1.)
					if((yr_nut .GE. Ys(jj)) .AND. (yr_nut .LT. Ys(jj+1))) then
						...
						wr00 = 0.5d0*(w(i,jj,kk)     + w(i,jj,kk-1))
						wr10 = 0.5d0*(w(i,jj+1,kk)   + w(i,jj+1,kk-1)) 
						wr01 = 0.5d0*(w(i,jj,kk+1)   + w(i,jj,kk))
						wr11 = 0.5d0*(w(i,jj+1,kk+1) + w(i,jj+1,kk)) 

						vr00 = 0.5d0*(v(i,jj,kk)     + v(i,jj-1,kk))
						vr10 = 0.5d0*(v(i,jj+1,kk)   + v(i,jj,kk))
						vr01 = 0.5d0*(v(i,jj,kk+1)   + v(i,jj-1,kk+1))
						vr11 = 0.5d0*(v(i,jj+1,kk+1) + v(i,jj,kk+1))
						
						wr = bilinear_interp(yr_nut,zr_nut,Ys(jj),Ys(jj+1),Zs(kk),Zs(kk+1),wr00,wr10,wr01,wr11)
						vr = bilinear_interp(yr_nut,zr_nut,Ys(jj),Ys(jj+1),Zs(kk),Zs(kk+1),vr00,vr10,vr01,vr11)
						...
						dist_b = SQRT((zb_nut-z0_t)**2+(yb_nut-y0_t)**2) - r
							dist_b = DMAX1(0.d0,dist_b)
						
						uTau = SQRT((nut_sgs+nu) * uc_r / dist_r)
							uTau = DMAX1(uTau,1.d-6*U_inf)
						
						! Newton-Raphson iteration to solve Spalding WF
						NRit=0 ; NRerr=1.d0
						do while (NRtolerance < NRerr .AND. NRit < NRmaxIter)					
							kUu = DMIN1(Kappa*uc_r/uTau, 50.d0)
							fkUu = EXP(kUu) - 1.d0 - kUu*(1.d0 + 0.5d0*kUu)
							
							NR_f = -uTau*dist_r/nu + uc_r/uTau + invE*(fkUu - kUu*kUu*kUu/6.d0)
							NR_df = dist_r/nu + uc_r/(uTau*uTau)  + invE*kUu*fkUu/uTau
							step = NR_f / NR_df
							
							! Limit the change to 50% of the current value per iteration to prevent overshooting
							if (ABS(step) > 0.5d0*uTau) step = SIGN(0.5d0*uTau, step)
							uTau_ = uTau + step
						...
						uTau = DMAX1(0.d0,uTau)
						yplusR = uTau*dist_r/nu
						...
						if(yplusB .LE. yplusLam) then
							...
							uPlusB = DLOG(E_WM * yplusB)/Kappa
							! Calculate nut Wall function
							nut(i,j,k) = DMAX1(0.d0,(yplusB*nu/(uPlusB + ROOTVSMALL)) - nu)
						endif

						! Blending nut with WALE
						if(yplusB >= 100.d0) then
							weight = 1.d0
						else
							weight = 0.5d0 * (1.d0 - COS(PI * yplusB / 100.d0))
						end if

						nut(i,j,k) = (1.d0 - weight) * nut(i,j,k) + weight * nut_sgs

						! Sum of y+ for avg calculation
						yplusSUM = yplusSUM + yplusB
						ypluspts = ypluspts + 1
...
! if (myid==master .AND. i==1 .AND. (istep == 100 .OR. mod(istep,1000)==0)) then
! write(84,'(I6,7(3x,F15.10))') NRit,yplusR,yplusB,uPlusB,uTau,nut(i,j,k)
! endif
					endif
				enddo
			endif
...
!$acc end data 
	...
	if (wall_model=='ON' .AND. mod(istep,1000)==0) then
		call MPI_ALLREDUCE( yplusSUM, yplusSUM_, 1, MPI_REAL8, MPI_SUM, MPI_COMM_WORLD, ierr )
		call MPI_ALLREDUCE( ypluspts, ypluspts_, 1, MPI_INTEGER, MPI_SUM, MPI_COMM_WORLD, ierr )
		write(83,'(2(F15.10,3x))') time,yplusSUM_/ypluspts_
	endif
...
! do k=istart,iend
	...
	! do i=1,nx
		...
		! ! Calculate the surface parallel slope
		! theta_b = DATAN(-1.d0*(zb_u - z0_t)/(yb_u - y0_t))

	! ! Find the impossing face centers, ignoring vertical surface to avoid a division by zero
	! if((0.5d0*ETA(i,j,k)+ETA(i,j,k+1)) .LT. 0.5d0 .AND. ((zb_u-z0_t)**2 + (yb_u-y0_t)**2) .LT. (r+c_ref*dySml)**2) then

		! zr_u = (r+c_ref*dySml)*(zb_u-z0_t)/DSQRT((zb_u-z0_t)**2 + (yb_u-y0_t)**2) + z0_t
		! yr_u = (r+c_ref*dySml)*(yb_u-y0_t)/DSQRT((zb_u-z0_t)**2 + (yb_u-y0_t)**2) + y0_t
			...
			! if((zr_u .GE. Zs(kk)) .AND. (zr_u .LT. Zs(kk+1))) then
				...
				! do jj=jBgnVOS,jEndVOS
					! if((yr_u .GE. Ys(jj)) .AND. (yr_u .LT. Ys(jj+1))) then
						...
						! uc_r = DABS(wr*DCOS(theta_b) + vr*DSIN(theta_b))
						! !write(83,'(7(3x,F15.10))') zb_u,yb_u,zr_u,yr_u,wr,vr,uc_r


						! ! Calculate uTau at point R using Newton-Raphson iteration (= uTau at point B)
						! dist_r = c_ref*dySml
						! dist_b = DSQRT((zb_u-z0_t)**2+(yb_u-y0_t)**2) - r
							! dist_b = DMAX1(0.d0,dist_b)
						...
						! NRit=0 ; NRerr=1.d0
						! do while (NRtolerance < NRerr .AND. NRit < NRmaxIter)					
							! yplusR_ = (KappaRe + yplusR)/(1.d0 + DLOG(E_WM * yplusR))

							! NRerr = DABS((yplusR - yplusR_)/yplusR)					
							! NRit=NRit+1
							! yplusR = yplusR_
						! end do
							
						! yplusR = DMAX1(0.d0,yplusR)
...
! do k=istart,iend
	...
	! do i=1,nx
		...
		! ! Calculate the surface parallel slope
		! theta_b = DATAN(-1.d0*(zb_v - z0_t)/(yb_v - y0_t))
	...
	! if((0.5d0*ETA(i,j,k)+ETA(i,j+1,k)) .LT. 0.5d0 .AND. ((zb_v-z0_t)**2 + (yb_v-y0_t)**2) .LT. (r+c_ref*dySml)**2) then
		...
		! zr_v = (r+c_ref*dySml)*(zb_v-z0_t)/DSQRT((zb_v-z0_t)**2 + (yb_v-y0_t)**2) + z0_t
		! yr_v = (r+c_ref*dySml)*(yb_v-y0_t)/DSQRT((zb_v-z0_t)**2 + (yb_v-y0_t)**2) + y0_t	
			...
			! if((zr_v .GE. Zs(kk)) .AND. (zr_v .LT. Zs(kk+1))) then
				...
				! do jj=jBgnVOS,jEndVOS
					! if((yr_v .GE. Ys(jj)) .AND. (yr_v .LT. Ys(jj+1))) then
						...
						! ! Calculate uTau at point R using Newton-Raphson iteration (= uTau at point B)
						! dist_r = c_ref*dySml
						! dist_b = DSQRT((zb_v-z0_t)**2+(yb_v-y0_t)**2) - r
							! dist_b = DMAX1(0.d0,dist_b)
			...
			! if((zr_u .GE. Zs(kk)) .AND. (zr_u .LT. Zs(kk+1))) then
				...
				! do jj=jBgnVOS,jEndVOS
					! if((yr_u .GE. Ys(jj)) .AND. (yr_u .LT. Ys(jj+1))) then
						...
						! uc_r = wr*DCOS(theta_b) + vr*DSIN(theta_b)
						! !write(83,'(7(3x,F15.10))') zb_u,yb_u,zr_u,yr_u,wr,vr,uc_r
						...
						! KappaRe = DABS(Kappa*uc_r*dist_r/nu) ! Use ABS because KappaRe must be +, while uc_r may be +/-
						! yplusR = yplusLam
	 ...
	 !$acc loop seq
		do k_wp = 1, wp_k-1
			...
			do wp_j_low = 1, wp_low(k_wp)
				if     (DABS(y_wp_up(k_wp,wp_j_up)-y_wp_low(k_wp,wp_j_low)) .LT. (0.01d0)) then
			...
			do j_wpp = 1, wp_up(k_wp-1)
				if     (DABS(y_wp_up(k_wp,j_wp)-y_wp_up(k_wp-1,j_wpp)) .LT. (0.01d0)) then
			...
			do j_wpp = 1, wp_low(k_wp-1)
				if     (DABS(y_wp_low(k_wp,j_wp)-y_wp_low(k_wp-1,j_wpp)) .LT. (0.01d0)) then
	...
	! do j=jBgnVOS,jEndVOS
		...
		! elseif(wdist(j,k) .LE. SQRT(max_dist)) then 					! Damping
			...
			! if(uTau .GT. ROOTVSMALL) then			! to avoid a division by zero, and wall function is not needed when Urel = 0
				! NRit=0 ; NRerr=1.d0
				! do while (NRtolerance < NRerr .AND. NRit < NRmaxIter)					
					! kUu = DMIN1(Kappa*Urel/uTau,50.d0)
					! fkUu = EXP(kUu) - 1.d0 - kUu*(1.d0+0.5d0*kUu)
				...
				! if(wall_model == 'ON' .AND. wdist(j,k) .LE. SQRT(dzSml**2+dySml**2)) then			! Applied only on the nearest cells
					! nut(1,j,k) = DMAX1(0.d0,(uTau*uTau/(Urel/wdist(j,k) + ROOTVSMALL)) - nu)
				...
				! if(wdist(j,k) .LE. SQRT(dzSml**2+dySml**2)) then
					...
					! yplusMAX = DMAX1(yplusMAX,ypluss(j,k))
					! yplusMIN = DMIN1(yplusMIN,ypluss(j,k))
