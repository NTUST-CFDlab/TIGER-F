...
program main
   ...
   LES_mode                         = 'ON'                  ! the LES mode (default: WALE)
...
!															  SOR with a dynamic omega is used. Set omega bounds in SOR.f90 when necessary. (default = 1.1-1.9)
   ...
   time_stepping_by					= 'CFL'					! 'DT' = constant dt, 'CFL' = constant CFL
					...
					constant_cfl	= 0.2d0					! input cfl if time_stepping_by a constant cfl (use 0.2 if using 2nd/3rd order AB)
...
!call nvtxStartRange("Smagorinsky")
		 call CalculateWALEViscosity()  				! calculate WALE nut		*** Smagorinsky.f90 ***
