...
implicit none
   ...
   write(1,*) ' '
   write(1,*) '============================================================='
   write(1,*) 'number of processes(MPI)  = ',nproc
...
!   end if
   ...
   write(1,*) '	CFL z = ', REAL(U_inf*(1.d0+ABS(blade_alpha))*dt / dzsml),&
          '  , CFL y = ', REAL(U_inf*(1.d0+ABS(blade_alpha))*dt / dySml)
   else if(time_stepping_by=='CFL') then
		write(1,*) '	constant CFL z = ',REAL(constant_cfl)
   endif
   write(1,*) ' '
   ...
   write(1,*) '==============freestream-based wall estimates================'
   write(1,*) '*Cf    = ', REAL(Cf) ,'  '
   write(1,*) '*Tau   = ', REAL(tau) ,'  '
   write(1,*) '*U_Tau = ', REAL(ufric) ,'  '
   write(1,*) '*Y plus wall distance         = ', REAL(Yplus) ,'  '   
   write(1,*) '*Van Driest  damping function = ', REAL(fwall) ,'  ' 
   write(1,*) '(*) Evaluated at each cell during the simulation'
   write(1,*) '============================================================='
   write(1,*) ' '
   ...
   write(1,*) '>>>>> filer near wall'
   write(1,*) 'start at ',REAL(startfilerNW_time),'sec,  every ',REAL(filerNW_int),'sec'
   write(1,*) 'Domain clipping: '
   write(1,*) '	X = ',x_start,'<--->',x_end
   write(1,*) '	Y = ',y_start,'<--->',y_end
   write(1,*) '	Z = ',z_start,'<--->',z_end
   write(1,*) ' '
   write(1,*) '	RHO   = distance to wall'
   write(1,*) '	RHO-U = yPlus'
   write(1,*) '	RHO-V = uPlus'
   write(1,*) '	RHO-W = nut'
   write(1,*) '	E     = ETA'   
   else if(filer_near_wall=='OFF') then; write(1,*) '>>>>> filer near wall: OFF'
   end if
   write(1,*) ' '
...
!   write(1,*) 'velocity Residual =',REAL(zeta_vel)
   ...
   ! write(31,*) ' TITLE     = "" '
   ! write(31,*) ' VARIABLES = t*,AOA,C<sub>T</sub>,C<sub>P</sub>out,C<sub>P</sub>in,C<sub>P</sub>nett,T<sub>gen</sub>,P<sub>gen</sub>'


   ! Filer Cp (only for a cylinder ------------------------------
   ...
   ! For individual blade Magnus VAWT----------------------------
   ! open (72,file='cd_time1.dat',position='append')
   ! write(72,*) ' TITLE     = "" '
   ! write(72,*) ' VARIABLES = t*,C<sub>D</sub>,C<sub>L</sub> '
 
   ! open (73,file='cd_time2.dat',position='append')
   ! write(73,*) ' TITLE     = "" '
   ! write(73,*) ' VARIABLES = t*,C<sub>D</sub>,C<sub>L</sub> ' 

   ! open (74,file='cd_time3.dat',position='append')
   ! write(74,*) ' TITLE     = "" '
   ! write(74,*) ' VARIABLES = t*,C<sub>D</sub>,C<sub>L</sub> ' 

   ! open (75,file='ct_time1.dat',position='append')
   ! write(75,*) ' TITLE     = "" '
   ! write(75,*) ' VARIABLES = t*,AOA,C<sub>T</sub>,C<sub>P</sub>out,C<sub>P</sub>in'
   
   ! open (76,file='ct_time2.dat',position='append')
   ! write(76,*) ' TITLE     = "" '
   ! write(76,*) ' VARIABLES = t*,AOA,C<sub>T</sub>,C<sub>P</sub>out,C<sub>P</sub>in'

   ! open (77,file='ct_time3.dat',position='append')
   ! write(77,*) ' TITLE     = "" '
   ! write(77,*) ' VARIABLES = t*,AOA,C<sub>T</sub>,C<sub>P</sub>out,C<sub>P</sub>in'
   ! ------------------------------------------------------------


   ! For individual tandem cylinder/airfoils---------------------
   ! open (87,file='Solid_information_1.dat',position='append')
   ! write(87,*) ' TITLE     = "" '
   ! write(87,'(A)') ' VARIABLES = t*,y,v_solid,cDrag,cLift,cPower'


   ! open (88,file='Solid_information_2.dat',position='append')
   ! write(88,*) ' TITLE     = "" '
   ! write(88,'(A)') ' VARIABLES = t*,y,v_solid,cDrag,cLift,cPower'
   ! ----------------------------------------------------------
...
implicit none
   ...
   write(21,'(3(3X,G18.10E2))') time, cDrag, cLift
   !write(21,*) time, AOA, cDrag, cLift

   ! write(31,'(8(3X,G18.10E2))') time, AOA, cTorq, cPower, cPower_in, cPower_nett, T_gen, P_gen

   ! For individual blade Magnus VAWT----------------------------
   ! write(72,'(3(3X,G18.10E2))') time, cDrag1, cLift1  
   ! write(73,'(3(3X,G18.10E2))') time, cDrag2, cLift2 
   ! write(74,'(3(3X,G18.10E2))') time, cDrag3, cLift3 
   ! write(75,'(5(3X,G18.10E2))') time, AOA, cTorq1, cPower1, cPower1_in
   ! write(76,'(5(3X,G18.10E2))') time, AOA, cTorq2, cPower2, cPower2_in
   ! write(77,'(5(3X,G18.10E2))') time, AOA, cTorq3, cPower3, cPower3_in
   ! ------------------------------------------------------------


   ! For individual tandem cylinder/airfoils---------------------
   ! write(87,'(6(3X,G18.10E2))') time, y0_t1, v_solid_1, cDrag1, cLift1, cPower1
   ! write(88,'(6(3X,G18.10E2))') time, y0_t2, v_solid_2, cDrag2, cLift2, cPower2
   ! ------------------------------------------------------------   
...
!$acc	u_TimeAvg(:,:,istart-2:iend+2),v_TimeAvg(:,:,istart-2:iend+2),w_TimeAvg(:,:,istart-2:iend+2),p_TimeAvg(:,:,istart-2:iend+2))
   ...
   !$OMP PARALLEL DO               PRIVATE(uc_,vc_,wc_,FlucS_u,FlucS_v,FlucS_w,FlucS_Sum) COLLAPSE(3)
...
!$acc	RS_vv(:,:,istart-2:iend+2),RS_vw(:,:,istart-2:iend+2),RS_ww(:,:,istart-2:iend+2))
   ...
   tnew = 1.d0/(time-startfilerAvg_time)
   told = time-dt-startfilerAvg_time
   !$OMP PARALLEL DO               PRIVATE(uc_,vc_,wc_) COLLAPSE(3)
...
implicit none
   ...
   open (17,file=TRIM('output3D/')//TRIM(filename)//fileformat,form='unformatted',access='stream')
   write(17) nblocks
...
implicit none
   ...
   !$OMP DO               REDUCTION(+: p_spansum,uc_spansum,vc_spansum,wc_spansum) COLLAPSE(2)
   !$acc parallel
   ...
   open (20,file=TRIM('output2D/')//TRIM(filename)//fileformat,form='unformatted',access='stream')
   write(20) nblocks
   write(20) 1, ny, nz
...
implicit none
   ...
   !$acc loop independent collapse(3)
   do k=k_start,k_end; do j=j_start,j_end; do i=i_start,i_end
      FXc(i,j,k) = 0.5d0*( FX(i,j,k)+FX(i-1,j,k) )
      FYc(i,j,k) = 0.5d0*( FY(i,j,k)+FY(i,j-1,k) )
      FZc(i,j,k) = 0.5d0*( FZ(i,j,k)+FZ(i,j,k-1) )
   enddo; enddo; enddo
   !$OMP END DO
	...
	do k=k_start,k_end; do j=j_start,j_end; do i=i_start,i_end
      Qout(i,j,k,1) = p(i,j,k)
      Qout(i,j,k,2) = -FXc(i,j,k)
      Qout(i,j,k,3) = -FYc(i,j,k)
      Qout(i,j,k,4) = -FZc(i,j,k)
      Qout(i,j,k,5) = ETA(i,j,k)
	enddo; enddo; enddo
	!$acc end parallel
	!$OMP END DO   
	...
	open (59,file=TRIM('output3D/')//TRIM(filename)//fileformat,form='unformatted',access='stream')
	write(59) nblocks
...
implicit none
	...
	open (60,file=TRIM('output3D/')//TRIM(filename)//fileformat,form='unformatted',access='stream')
	write(60) nblocks
...
implicit none
   ...
   if(solid_motion == 0 .OR. time < StartDynamic_time+dt) then
      open (unit=18,file='STATIC.bak',form='unformatted',access='stream')
   else 
      open (unit=18,file='DYNAMIC.bak',form='unformatted',access='stream')
   endif
   write(18) nblocks
   write(18) nx, ny, nz
   ...
   ! write(18) totalFX1_,totalFY1_,totalFZ1_	! tandem cylinder
   ! write(18) totalFX2_,totalFY2_,totalFZ2_	! tandem cylinder
   ! write(18) u_solid_1,v_solid_1,w_solid_1	! tandem cylinder
   ! write(18) u_solid_2,v_solid_2,w_solid_2	! tandem cylinder
   ! write(18) x0_t1,y0_t1,z0_t1				! tandem cylinder
   ! write(18) x0_t2,y0_t2,z0_t2   			! tandem cylinder
   close(18)
...
implicit none
   ...
   open (unit=19,file='RUNNING_AVG.bak',form='unformatted',access='stream')
   write(19) nblocks
   write(19) nx, ny, nz
...
implicit none
   ...
   open (unit=23,file='RUNNING_AVG1.bak',form='unformatted',access='stream')
   write(23) nblocks
   write(23) nx, ny, nz
...
implicit none
   ...
   open (unit=24,file='RUNNING_AVG2.bak',form='unformatted',access='stream')
   write(24) nblocks
   write(24) nx, ny, nz
