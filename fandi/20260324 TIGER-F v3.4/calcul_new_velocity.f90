...
!$acc data present(X,Y,Z,Xs,Ys,Zs,Dxs,Dys,Dzs,ETA,p(:,:,istart-2:iend+2), &
...
!$acc create(u1(:,:,istart-2:iend+2),v1(:,:,istart-2:iend+2),w1(:,:,istart-2:iend+2))
  ...
  !$OMP PARALLEL
   !$OMP DO               PRIVATE(ETA_sub,u_solid1) COLLAPSE(3)
  !$acc parallel
   ...
   do j=1,ny;do i=1,nx
      ...
      u1(i,j,k) = u0(i,j,k) - dt*(p(i+1,j,k)-p(i,j,k))/den_flu / Dxs(i)
		...
		ETA_sub = 0.5d0*(ETA(i,j,k)+ETA(i+1,j,k))
				...
				!***************************************!
				!Face location legend:					!
				!X = X(i+1) ; Y = Ys(j)	; Z = Zs(k)		!
				!***************************************!
   ...
   !$OMP DO               PRIVATE(ETA_sub,v_solid1) COLLAPSE(3)
   ...
   do j=1,ny;do i=1,nx
      ...
      v1(i,j,k) = v0(i,j,k) - dt*(p(i,j+1,k)-p(i,j,k))/den_flu / Dys(j)
		...
		ETA_sub = 0.5d0*(ETA(i,j,k)+ETA(i,j+1,k))
				...
				!***************************************!
				!Face location legend:					!
				!X = Xs(i) ; Y = Y(j+1)	; Z = Zs(k)		!
				!***************************************!


	!Set the total imposed and 2way velocity (rotor + 1blade rotation)---------------------------------------------------

	   if( SQRT((Zs(k)-z0_t)**2 + (Y(j+1)-y0_t)**2) .LT. (blade_r+0.05d0)) then
	...
	!Set the total imposed and 2way velocity (rotor + 2blade rotation)---------------------------------------------------
	   ...
	   ! if( SQRT((Zs(k)-z0_t1)**2 + (Y(j+1)-y0_t1)**2) .LT. (blade_r+0.05d0)) then
			 ! v_solid1= v_solid + rotor_omega * (Zs(k)-z0) + blade_omega * (Zs(k)-z0_t1) 	! blade 1, Positive omega is CCW
	   ! else if( SQRT((Zs(k)-z0_t2)**2 + (Y(j+1)-y0_t2)**2) .LT. (blade_r+0.05d0)) then
	...
	!Set the total imposed and 2way velocity (rotor + 3blade rotation)---------------------------------------------------
	   ...
	   ! else if( SQRT((Zs(k)-z0_t2)**2 + (Y(j+1)-y0_t2)**2) .LT. (blade_r+0.05d0)) then
			 ! v_solid1= v_solid + rotor_omega * (Zs(k)-z0) + blade_omega * (Zs(k)-z0_t2) 	! blade 2, Positive omega is CCW 
	   ! else if( SQRT((Zs(k)-z0_t3)**2 + (Y(j+1)-y0_t3)**2) .LT. (blade_r+0.05d0)) then
	...
	! Set the total imposed and 2way velocity (VIV tandem Cylinders)---------------------------------------------------
	  ! if( Zs(k) .LT. Zsplit_pos) then
		  ! v_solid1 = v_solid_1		!Cylinder1
	  ! else
		  ! v_solid1 = v_solid_2		!Cylinder2	
	  ! endif
	! --------------------------------------------------------------------------------------------------------------------
   ...
   !$OMP DO               PRIVATE(ETA_sub,w_solid1) COLLAPSE(3)
   ...
   do j=1,ny;do i=1,nx
      ...
      w1(i,j,k) = w0(i,j,k) - dt*(p(i,j,k+1)-p(i,j,k))/den_flu  / Dzs(k)
		...
		ETA_sub = 0.5d0*(ETA(i,j,k)+ETA(i,j,k+1))
				...
				!***************************************!
				!Face location legend:					!
				!X = Xs(i) ; Y = Ys(j)	; Zs = Z(k+1)	!
				!***************************************!


	!Set the total imposed and 2way velocity (rotor + 1blade rotation)---------------------------------------------------

	   if( SQRT((Z(k+1)-z0_t)**2 + (Ys(j)-y0_t)**2) .LT. (blade_r+0.05d0)) then
	...
	!Set the total imposed and 2way velocity (rotor + 2blade rotation)---------------------------------------------------
	   ...
	   ! if( SQRT((Z(k+1)-z0_t1)**2 + (Ys(j)-y0_t1)**2) .LT. (blade_r+0.05d0)) then
			 ! w_solid1= w_solid + (-1.d0 * rotor_omega) * (Ys(j)-y0) + (-1.d0 * blade_omega) * (Ys(j)-y0_t1)	! blade 1, Positive omega is CCW
	   ! else if( SQRT((Z(k+1)-z0_t2)**2 + (Ys(j)-y0_t2)**2) .LT. (blade_r+0.05d0)) then
	...
	!Set the total imposed and 2way velocity (rotor + 3blade rotation)---------------------------------------------------
	   ...
	   ! else if( SQRT((Z(k+1)-z0_t2)**2 + (Ys(j)-y0_t2)**2) .LT. (blade_r+0.05d0)) then
			 ! w_solid1= w_solid + (-1.d0 * rotor_omega) * (Ys(j)-y0) + (-1.d0 * blade_omega) * (Ys(j)-y0_t2)	! blade 2, Positive omega is CCW
	   ! else if( SQRT((Z(k+1)-z0_t3)**2 + (Ys(j)-y0_t3)**2) .LT. (blade_r+0.05d0)) then
...
!$acc data present(Dxs,Dys,Dzs,u(:,:,istart-2:iend+2),v(:,:,istart-2:iend+2),w(:,:,istart-2:iend+2), &
!$acc	u2(:,:,istart-2:iend+2),v2(:,:,istart-2:iend+2),w2(:,:,istart-2:iend+2))

    maxvel = 0.d0
    maxvel_= 0.d0
   !$OMP PARALLEL DO               PRIVATE(u_x,v_y,w_z) REDUCTION(max:maxvel) COLLAPSE(3) 
   !$acc parallel loop independent private(u_x,v_y,w_z) reduction(max:maxvel) collapse(3) gang vector
   do k=istart,iend  
   do j=1,ny;do i=1,nx
	  ...
	  u_x = ABS(u(i,j,k)/Dxs(i))	! Max velocity search for constant CFL (CFL = dt * u/dx)
	  v_y = ABS(v(i,j,k)/Dys(j))
	  w_z = ABS(w(i,j,k)/Dzs(k))
	
	maxvel = MAX(maxvel,u_x,v_y,w_z)
