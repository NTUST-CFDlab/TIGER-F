...
!      call gridder_read_txt()          ! grid coordinate is supplied (grid.txt) in X(i),Y(j),Z(k) format
   ...
   !----------------------------------------------------------------!
	...
	istep = 0
	istep2= 0
    nstep_round	= (nstep / 10) * 10					! Round nstep to tens to calculate simulation progess
...
!call nvtxEndRange
      ...
      !--------------------Exit on diverged--------------------!			! Emergency stop if pressure solver diverges
		if (myid==master .AND. pChangeMax_>huge(0.0)) then
			call date_and_time(date=date_str, time=time_str)						
			write(73,'(1X,A4,"-",A2,"-",A2," ",A2,":",A2,":",A2,A,I7,A,F9.4,A)') &
				...
				' , time = ', real(time), ' sec'
			write(73,*) ' '	
			write(73,'(A,F9.4,A)') ' Last backup time = ',real(lastbackuptime),' sec'
			write(73, '(/)')
			stop
		end if		 
      !--------------------------------------------------------!
...
!call nvtxStartRange("output files")
	if ((filer3d==1 .AND. mod(istep,isto3d)==0) .OR. (filer2d/=0 .AND. mod(istep,isto2d)==0) .OR. &
...
! --------------------------------------------------------------------------------
      ...
      !---------------print simulation progress---------------!
		if (myid == master .AND. istep==NINT(2./dt)) then
			timeapproxstart = MPI_WTIME()
		endif
		
		if (myid == master .AND. istep==NINT(10./dt)) then
			timeapproxend = MPI_WTIME()
			write(73,'(1X,19("*"),A,F8.4,A)') ' Total cost time (Approx.)= ', &
				REAL((timeapproxend-timeapproxstart)*(nstep-NINT(10./dt))/NINT(8./dt)/3600. +totalcosttime/3600.),' Hr'
			write(73,*) ' '
			flush(73)		
		endif
		
		if (myid == master .AND. MOD(istep, nstep_round/10)==0) then
			progress = INT(100.0*REAL(istep)/REAL(nstep_round))
			call date_and_time(date=date_str, time=time_str)
			write(73,'(1X,A4,"-",A2,"-",A2," ",A2,":",A2,":",A2,A,I4,A,2(F8.4,A))') &
				date_str(1:4), date_str(5:6), date_str(7:8), &
				time_str(1:2), time_str(3:4), time_str(5:6), ' ', &
				progress,'% completed, elapsed = ', &
				REAL(totalcosttime/3600.) ,' Hr, remaining = ', &
