...
MODULE variables
	...
	real*8, parameter			 :: rotor_tsr			= 0.6d0					! + = CCW
    ...
    ! real*8 :: Ur_star 			= (fixed_Re - 900.d0)*16.d0/(15000.d0 - 900.d0) + 1.d0
	
	! real*8 :: k_spring 			= (2.d0 * PI / ((fixed_Re - 900.d0)*16.d0/(15000.d0 - 900.d0) + 1.d0)) ** 2
	
	! real*8 :: c_damper 			= 4.d0 * PI * 0.0054d0 / ((fixed_Re - 900.d0)*16.d0/(15000.d0 - 900.d0) + 1.d0)
    ...
    real*8                       :: Y_castingarea, Z_castingarea, X_castingarea, solid_volume, I_moment
    ...
    real*8, dimension(:,:,:), allocatable, pinned 	 :: yplus_print,uplus_print,dist_print		! NVIDIA compiler
    !real*8, dimension(:,:,:), allocatable		 	 :: yplus_print,uplus_print,dist_print		!  INTEL compiler
    ...
    real*8           								 :: dt, constant_dt, constant_cfl
...
!
    ...
    real*8                                         :: totalFZ1, totalFY1, totalFZ2, totalFY2, totalFZ3, totalFY3

    real*8                                         :: totalFX_, totalFY_, totalFZ_

    real*8                                         :: totalFZ1_, totalFY1_, totalFZ2_, totalFY2_, totalFZ3_, totalFY3_
	...
	integer, parameter			 :: nclps = 1 ! use 2 collapsed loops if nz/nproc/nthread < 3 (Only for OpenMP)
    ...
    character(len=8)         :: filer_virtual_force,filer_running_avg
	
    real*8					 :: backup_int, coeff_start
	
	real*8                   :: startfiler3d_time, startfiler2d_time, startfilerVF_time, startfilerAvg_time

	real*8                   :: filer3d_int, filer2d_int, istocp_int, filerVF_int, filerAvg_int, filerCp_int

	real*8                   :: next_filer3d, next_filer2d, next_filerVF, next_filerAvg

	real*8                   :: next_backup, next_backupAvg, next_filerCp, next_filerWall
