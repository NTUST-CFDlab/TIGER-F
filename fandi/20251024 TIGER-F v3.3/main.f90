...
!															  SOR with a dynamic omega is used. Set omega bounds in SOR.f90 when necessary. (default = 1.1-1.9)
   ...
   time_stepping_by					= 'cfl'					! 'dt' = constant dt, 'cfl' = constant CFL
					...
					constant_cfl	= 0.4d0					! input cfl if time_stepping_by a constant cfl
   ...
   filer2d							= 'AVG'					! create 2D output (2d_xxxx.q). 'AVG'/'ON' : Spanwise avg ; 'MID' : Spanwise Midplane slice
			startfiler2d_time		= 50.d0					! initialize time of writing filer2d data
   ...
   filer_running_avg				= 'OFF'					! Calculate running average of u,v,w,p,TKE for all timesteps
...
!      call gridder_read_txt()          ! grid coordinate is supplied (grid.txt) in X(i),Y(j),Z(k) format
   ...
   if (filer_running_avg=='ON' .AND. resume=='ON' .AND. time > startfilerAvg_time) then												
...
!$acc	v_TimeAvg(:,:,istart-2:iend+2), w_TimeAvg(:,:,istart-2:iend+2), p_TimeAvg(:,:,istart-2:iend+2)) 	if(filer_running_avg=='ON')
...
!call nvtxEndRange     
   !time5end = MPI_WTIME()     
      ...
      !---------- Calculate running average of u,v,w,p,TKE ----------!
		...
		if (filer_running_avg=='ON' .AND. time .GE. startfilerAvg_time) then
...
! ------------------------------- TKE output files ----------------------------------
	...
	if (filer_running_avg=='ON' .AND. time > startfilerAvg_time-dt .AND. (time >= next_filerAvg-1.0d-10 .OR. time >= next_backupAvg-1.0d-10)) then        			!   modified 17_10_2024
...
!$acc exit data if(filer_running_avg=='ON')
!$acc end data
