...
!$OMP PARALLEL DO PRIVATE(j,i,zb_nut,yb_nut,zr_nut,yr_nut,theta_b,kk,jj,wra,wrb,wrc,wrd,vra,vrb,vrc,vrd,wr_ceil,wr_floor,wr,vr_ceil,vr_floor,vr,uc_r,dist_r,dist_b,KappaRe,yplusR,yplusR_,NRit,NRerr,uTau,yplusB,uPlusB,uc_b) collapse(nclps)
!$acc parallel
...
! !$OMP PARALLEL DO PRIVATE(j,i,zb_u,yb_u,zr_u,yr_u,theta_b,kk,jj,wra,wrb,wrc,wrd,vra,vrb,vrc,vrd,wr_ceil,wr_floor,wr,vr_ceil,vr_floor,vr,uc_r,dist_r,dist_b,KappaRe,yplusR,yplusR_,NRit,NRerr,uTau,yplusB,uPlusB,uc_b) collapse(nclps)
! !$acc parallel
...
! !$OMP PARALLEL DO PRIVATE(j,i,zb_v,yb_v,zr_v,yr_v,theta_b,kk,jj,wra,wrb,wrc,wrd,vra,vrb,vrc,vrd,wr_ceil,wr_floor,wr,vr_ceil,vr_floor,vr,uc_r,dist_r,dist_b,KappaRe,yplusR,yplusR_,NRit,NRerr,uTau,yplusB,uPlusB,uc_b) collapse(nclps)
! !$acc parallel
...
! DISCLAIMER: this is only for extruded span solids!!!
...
!$OMP PARALLEL DO PRIVATE(wdistance,wdistance_,hyp,wsin_,wcos_,k_wp,wp_j_up,wp_j_low,j_wp,j_wpp) collapse(2)
!$acc parallel
...
!$OMP PARALLEL DO PRIVATE(j,Ucp,Uwp,Urel,uTau,uTau_,NRit,NRerr,kUu,fkUu,NR_f,NR_df) reduction(+: yplusss,countYplus) reduction(MAX: yplusMAX) reduction(MIN: yplusMIN) collapse(nclps)
	...
	do j=jBgnVOS,jEndVOS
		...
		elseif(wdist(j,k) .LE. SQRT(max_dist)) then 					! Damping
			...
			if(uTau .GT. ROOTVSMALL) then			! to avoid a division by zero, and wall function is not needed when Urel = 0
				...
				elseif(Damping_F=='ON' .AND. ypluss(j,k) .LT. 300.d0) then
