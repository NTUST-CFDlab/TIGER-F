...
!$OMP			  REDUCTION(min : az_min,ay_min) collapse(nclps)
...
!$OMP END PARALLEL DO  
	...
	!-------------- Calculate moment of inertia -------------------
		...
		if( istep == 0 )then
			...
			!$OMP PARALLEL DO PRIVATE(i) REDUCTION(+: I_moment) collapse(2)
...
!$OMP PARALLEL DO PRIVATE(j,Yf1,Zf1,Yf2,Zf2) REDUCTION(max : az_max,ay_max) &
!$OMP			  REDUCTION(min : az_min,ay_min) collapse(nclps)
!$acc parallel vector_length(32)
...
!$OMP			  REDUCTION(max : az_max,ay_max) REDUCTION(min : az_min,ay_min) collapse(nclps)
!$acc parallel vector_length(32)
...
! !$OMP PARALLEL DO PRIVATE(i,j,DIAGONAL,DISTANCE,SDIST,l,m,n,dxg,dyg,dzg,SX,SY,SZ,xi) collapse(nclps)
...
!$OMP			  REDUCTION(min : az_min,ay_min,ax_min) collapse(nclps)
!$acc parallel vector_length(32)
...
!$OMP PARALLEL DO PRIVATE(i,l,m,n,SX,SY,SZ,Xf,Yf,Zf,xi) collapse(2)
!$acc parallel vector_length(32)
...
!$OMP PARALLEL DO PRIVATE(i,l,m,n,nn,SX,SY,SZ,xi,sphere) collapse(2)
!$acc parallel vector_length(32)
